...
Skills live at ``src/litmus/skills/<skill>/SKILL.md`` (11 of them:
litmus-tests, litmus-mocks, litmus-stations, litmus-parts, litmus-profiles,
litmus-sites, litmus-capture, litmus-analysis, litmus-debug, litmus-interactive,
litmus-datasheets). The ``litmus-tests`` set is the original start-simple ladder
...
- env             — extra env vars for the pytest run (e.g. LITMUS_AUTO_CONFIRM=1)
...
- expect_cli      — structural check: the candidate's output/files must contain
                     this literal ``litmus <subcommand>`` invocation.
...
TASKS = [
    # ── litmus-tests — Rung 0 — zero config ─────────────────────────────────
    Task(
        "r0_observe",
    ...
    # ── litmus-tests — Rung 2 — instruments (mock) ───────────────────────────
    Task(
        "r2_instruments",
    ...
    # ── litmus-tests — over-engineering traps (graded by ABSENCE) ───────────
    Task(
        "m1_overeng_log",
    ...
    Task(
        ...
        "Capture a scope waveform (an array of samples) in a Litmus test and "
        "read it back within the same test to confirm it was stored.",
        rung=1,
        skill="litmus-capture",
    ),
    # ── litmus-analysis ──────────────────────────────────────────────────────
    Task(
        ...
        rung=0,
        skill="litmus-analysis",
        requires_pytest=False,
        expect_cli="litmus metrics summary",
    ),
    # ── litmus-debug ─────────────────────────────────────────────────────────
    Task(
    ...
    Task(
        ...
        rung=2,
        skill="litmus-interactive",
        env={"LITMUS_AUTO_CONFIRM": "1"},
    ),
    # ── litmus-datasheets (manual — needs a real PDF fixture) ───────────────
    Task(
        "datasheets_import_part",
        "Import a part datasheet PDF into a part spec and a starter test.",
        rung=3,
        skill="litmus-datasheets",
        manual=True,
