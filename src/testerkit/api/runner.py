...
from testerkit.api.models import ActiveRun, LaunchRequest, RunStatus
...
class TestRunner:
    ...
    async def _run_tests(self, run_info: RunInfo) -> None:
        ...
        env["_TESTERKIT_SESSION_ID"] = run_info.run_id
        # Enable mock instruments if requested
        if req.mock_instruments:
            env["TESTERKIT_MOCK_INSTRUMENTS"] = "1"
...
def get_runner() -> TestRunner:
    ...
    On first access we resolve the project's data dir (``testerkit.yaml``
