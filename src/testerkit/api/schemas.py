...
Field names mirror the write-side event hierarchy in testerkit/data/events.py:
...
class InstrumentView(BaseModel):
    ...
    Mirrors InstrumentConnected (testerkit/data/events.py), minus EventBase fields.
    """
...
class MeasurementView(BaseModel):
    ...
    Mirrors MeasurementRecorded (testerkit/data/events.py), minus step context
...
class StepView(BaseModel):
    ...
    Mirrors StepStarted + StepEnded folded together (testerkit/data/events.py).
    """
...
class RunView(BaseModel):
    ...
    Mirrors RunStarted + RunEnded folded together (testerkit/data/events.py).
...
def build_run_view(
    ...
    Args:
        run: ``RunRow`` from ``RunsQuery.get(run_id)`` — the run-level
            ...
            need ``testerkit.analysis.runs_query``).
...
def load_run_view(
    ...
    *,
    ...
    - ``testerkit`` MCP tool's ``get`` action
    - report generation in ``testerkit.reports``
