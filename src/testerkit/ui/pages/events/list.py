...
from testerkit.ui.shared.services import query_events

# Curated event-type list. The actual event store can hold any value
# (see ``src/testerkit/data/events.py`` for the full ~25-class set); the
...
def _summarize(event: dict[str, Any]) -> str:
    ...
    if et in ("run.started", "run.ended"):
        return event.get("uut_serial_number") or event.get("station_id") or ""
