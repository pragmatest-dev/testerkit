...
def format_datetime(dt: datetime | str | None) -> str:
    ...
    return f'<span class="testerkit-time" data-utc="{iso}">{fallback}</span>'
...
def lookup_session_label(session_id: str) -> tuple[str, bool]:
    ...
    * A ``found=False`` result is sticky for the same lifetime — if a
    ...
    """
    from testerkit.ui.shared.services import query_sessions
...
def lookup_run_label(run_id: str) -> tuple[str, bool]:
    ...
    """
    from testerkit.ui.shared.services import get_recent_runs
...
def local_time_init_script() -> str:
    """Return the JS that converts every ``.testerkit-time`` span on the
    ...
    return """
    (() => {
      if (window.__testerkit_time_init) return;
      window.__testerkit_time_init = true;
      ...
      const sweep = () => {
        document.querySelectorAll('.testerkit-time:not([data-formatted])').forEach(fmt);
      };
      sweep();
...
def local_date_input_init_script() -> str:
    ...
    * ``window.testerkitLocalToUtcDate(dateStr)`` — converts a YYYY-MM-DD or
    ...
    * ``window.testerkitUtcToLocalDate(dateStr)`` — inverse: a UTC YYYY-MM-DD
    ...
    * ``window.testerkitLocalToUtcDateTime(dtStr)`` — converts a local datetime
      string (e.g. ``"2026-01-15T08:30:00"``) to a UTC ISO-8601 string.
    * ``window.testerkitUtcToLocalDateTime(utcDtStr)`` — inverse: UTC ISO-8601
    ...
    value of every ``<div class="testerkit-date-utc" data-utc="YYYY-MM-DD">``
    wrapper created by :func:`utc_date_input`.  Mirrors :func:`local_time_init_script`
    which does the same for ``.testerkit-time`` display spans — Python emits the
    UTC value; conversion is entirely in this JS layer.

    Called from :func:`~testerkit.ui.shared.layout.create_layout` so all functions
    and the observer are available on every page that hosts date inputs.
    """
    return r"""
    window.testerkitLocalToUtcDate = function(localDateStr) {
    ...
    window.testerkitUtcToLocalDate = function(utcDateStr) {
    ...
    window.testerkitLocalToUtcDateTime = function(dtStr) {
      if (!dtStr) return '';
      try {
    ...
    window.testerkitUtcToLocalDateTime = function(utcDtStr) {
      if (!utcDtStr) return '';
      try {
    ...
    (function() {
      if (window.__testerkit_date_input_init) return;
      window.__testerkit_date_input_init = true;
      function convertDateInputs() {
        document.querySelectorAll('.testerkit-date-utc[data-utc]').forEach(function(el) {
          if (el.dataset.localConverted) return;
          var utcDate = el.dataset.utc;
          if (!utcDate) { el.dataset.localConverted = '1'; return; }
          var localDate = window.testerkitUtcToLocalDate(utcDate);
...
def utc_date_input(
    ...
    *,
    ...
    :func:`~testerkit.ui.shared.layout.create_layout` — the same injection
    ...
    _text_js = "(...args) => emit(args[0] ? window.testerkitLocalToUtcDate(args[0]) : '')"
    ...
    _date_js = (
        "(...args) => {"
        "  var local = args[0] || '';"
        "  var utc = local ? window.testerkitLocalToUtcDate(local) : '';"
        "  emit(utc);"
    ...
    # local_date_input_init_script() queries .testerkit-date-utc[data-utc] and
    # uses that attribute to look up the UTC value to convert for display.
    with ui.element("div").classes(f"testerkit-date-utc {uid}").props(f'data-utc="{value or ""}"'):
...
def utc_datetime_input(
    ...
    *,
    ...
    ``window.testerkitLocalToUtcDateTime`` / ``window.testerkitUtcToLocalDateTime``
    ...
    Note:
        No page in the current TesterKit operator UI uses ``utc_datetime_input``
    ...
    _inp = (
        ...
        .classes(f"{classes} testerkit-datetime-utc")
    ...
    _inp.on(
        "update:model-value",
        _py_handler,
        js_handler="(...args) => emit(args[0] ? window.testerkitLocalToUtcDateTime(args[0]) : '')",
    )
    return handle
...
def info_field(label: str, value: str) -> None:
    ...
    emits a ``<span class="testerkit-time">`` so the browser-local
...
def page_header(
    ...
    *,
    ...
    """TesterKit design-system page-header strip.
...
def stat_card(value: str, label: str, color_class: str = "text-slate-700") -> None:
    """TesterKit stat card — big value on top, small label below.
...
def page_layout(*, padding: str = "p-6", gap: str = "gap-4"):
    """TesterKit design-system page shell — viewport-bound flex column.
    ...
    Usage::
        ...
        def results_page():
            ...
            with page_layout():
                ...
                ...
    """
    return ui.column().classes(f"testerkit-page {padding} {gap}")
...
def subscribe_with_refresh(
    ...
    *,
    ...
    Args:
        event_store: A :class:`testerkit.data.event_store.EventStore`
    ...
    from testerkit.ui.shared.event_binding import ui_subscribe
...
STICKY_TABLE_CSS = """
.testerkit-sticky-table {
    overflow: auto;
}
.testerkit-sticky-table tr th {
    position: sticky;
    top: 0;
    z-index: 2;
    background: white;
...
def testerkit_table(
    ...
    *,
    ...
    """Create a dense flat table with standard testerkit styling.
    ...
    return (
        ...
        .classes("w-full testerkit-table")
...
def data_table(
    ...
    *,
    ...
    """Canonical TesterKit table — every list view goes through this.
    ...
    Args:
        ...
        time_columns: Names of columns whose cell values are HTML
            from :func:`format_datetime` (i.e. ``<span class="testerkit-time">``
    ...
    table = (
        ...
        .classes("w-full flex-1 min-h-0 testerkit-data-table")
