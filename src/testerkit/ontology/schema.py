"""Pydantic schema for the TesterKit ontology file.

The ontology YAML at ``src/testerkit/ontology/testerkit.yaml`` describes every
TesterKit concept, its canonical Pydantic model, and the relationships
between concepts. This module is the structural source of truth for
that file — same pattern as every other ``testerkit/models/*.py``: a
...
class VerbKind(StrEnum):
    ...
    this enum. See ``src/testerkit/ontology/testerkit.yaml`` for prose
    definitions of each verb.
    """

    # Domain composition — each verb carries TesterKit-specific meaning.
    # See src/testerkit/ontology/testerkit.yaml preamble for prose definitions.
...
class Ontology(BaseModel):
    """Root model for ``src/testerkit/ontology/testerkit.yaml``."""
