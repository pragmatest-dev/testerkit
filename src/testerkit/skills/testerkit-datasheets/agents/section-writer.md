...
- The orchestrator runs `testerkit.store.format_file_inplace()` after writing to enforce consistent formatting
...
`uv run python -c "from pathlib import Path; from testerkit.store import load_catalog_entry; load_catalog_entry(Path('{{YAML_PATH}}'))"`
