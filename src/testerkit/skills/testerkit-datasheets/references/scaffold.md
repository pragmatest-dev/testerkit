...
Use the catalog pipeline (`references/catalog-pipeline.md`) instead when:
...
- If you don't know the instrument well, say so and suggest the catalog pipeline (`references/catalog-pipeline.md`)
...
If confidence is LOW, recommend the catalog pipeline (`references/catalog-pipeline.md`) instead and stop.
</step>
...
- DMM: src/testerkit/catalog/generic/generic_dmm.yaml
- PSU: src/testerkit/catalog/generic/generic_psu.yaml
- Oscilloscope: src/testerkit/catalog/generic/generic_oscilloscope.yaml
- Electronic Load: src/testerkit/catalog/generic/generic_eload.yaml
</step>
...
Validate with:
```bash
uv run python -c "from pathlib import Path; from testerkit.store import load_catalog_entry; load_catalog_entry(Path('catalog/{manufacturer}/{manufacturer}_{model}.yaml'))"
```
</step>
...
- Run the catalog pipeline (`references/catalog-pipeline.md`) later for exact specs if needed
</step>
...
I recommend using the catalog pipeline (`references/catalog-pipeline.md`) with the part datasheet
