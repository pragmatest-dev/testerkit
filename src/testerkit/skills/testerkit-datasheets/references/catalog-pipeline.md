...
<step id="1.2">
Read `src/testerkit/models/enums.py`.
...
| Instructions | Read PDF, build section map, identify scaffold pages, verify page coverage | See testerkit/skills/agents/section-splitter.md for reference |
...
| Instructions | Read targeted pages, write device-level YAML, validate | See testerkit/skills/agents/scaffold-writer.md for reference |
</step>
...
| Instructions | Read PDF, write inventory | See testerkit/skills/agents/section-extractor.md for reference |
...
| Instructions | Read inventory, write YAML | See testerkit/skills/agents/section-writer.md for reference |
...
1. **Reviewer-fixer (opus agent):**
   ...
   | ENUM_LIST | MeasurementFunction enum values | Phase 1 |
   | Instructions | Review inventory vs YAML, fix issues directly | See testerkit/skills/agents/section-reviewer.md for reference |
...
```
uv run python -c "from testerkit.store import format_file_inplace; from pathlib import Path; format_file_inplace(Path('YAML_PATH'))"
uv run testerkit validate --type catalog YAML_PATH
```
