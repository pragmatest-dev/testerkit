...
description: Create hardware tests from a part datasheet using TesterKit MCP tools. Guides through spec extraction, station setup, and test generation with step-by-step approval.
---

# Datasheet to Test Workflow

<overview>
Create hardware tests from a part datasheet using TesterKit.
...
| Tool | Purpose |
|------|---------|
| testerkit_project(action="init", path="...") | Initialize project, returns project_root |
| testerkit_project(action="save", type="...", id="...", content={...}, project=...) | Save part/station/test |
| testerkit_project(action="read", path="...", project=...) | Read files or templates |
| testerkit_match(part_id, station_id, project) | Check compatibility |
| testerkit_run(test="...", station="...", serial="...", project=...) | Execute tests |
| testerkit_open(type="...", id="...") | Get browser URL for viewing/editing |
| testerkit_project(action="lookup_enum", id="FRES") | Resolve datasheet abbreviation to enum value |
| testerkit_project(action="enum_reference") | Full enum abbreviation table (markdown) |
...
<step id="1.1">
Ask the user where to create the project — suggest ~/testerkit-<part_number> but let them choose.
</step>

<step id="1.2">
Initialize project: testerkit_project(action="init", path="<user's chosen path>")
</step>
...
<step id="1.4">
Use testerkit_project(action="lookup_enum", id="...") to resolve datasheet abbreviations
...
Use ``testerkit_project(action="schema", type="part")`` to fetch the live
...
<step id="2.2">
Save with testerkit_project(action="save", type="part", ...) — schema is validated server-side.
</step>
...
<step id="2b.2">
Call testerkit_match(part_id="<part_id>", project=project_root) —
the platform derives requirements from the saved part characteristics automatically.
Do NOT build requirements manually.
</step>

<step id="2b.3">
If testerkit_match returns catalog recommendations, present them.
...
Generic instruments are in src/testerkit/catalog/generic/:
...
<step id="3.1">
Use the instruments selected in Phase 2b, or use testerkit_discover().
</step>

<step id="3.2">
Build station config with realistic mock values — schema is
validated server-side. Use ``testerkit_project(action="schema", type="station")``
for the live shape.
</step>
...
<step id="4.1">
Generate **pytest-native** test code. Tests are plain pytest — no decorator, no base class. Use the TesterKit fixtures (`context`, `verify`, `observe`) and the single `testerkit_limits` marker for inline limits. Sweeps use native `@pytest.mark.parametrize` or the `testerkit_sweeps` marker; see the project's `examples/` chapters for full patterns.
...
- `observe` / `verify` / `stream` are the three test-author verbs — see `testerkit docs show concepts/data/three-verbs`
...
<step id="5.2">
Run with testerkit_run(test="tests/test_x.py", station="station_id", serial="SERIAL", project=project_root).
</step>
...
- Characteristics: use ``testerkit_project(action="schema", type="capability")`` to discover valid MeasurementFunction values. Set `function:` + `direction:` (input/output).
