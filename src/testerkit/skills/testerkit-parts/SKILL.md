...
or a sidecar `limits:` entry (`testerkit-tests`) covers that. Reach for a part
...
`testerkit-tests`. Don't duplicate the guardband math here; write the
...
```bash
testerkit validate parts/buck_3v3.yaml
testerkit validate                         # scans parts/, stations/, fixtures/, catalog/
```
...
```python
testerkit_schema(yaml_type="part")
testerkit_project(action="save", type="part", id="buck_3v3", content={...}, project=project_root)
```
...
Read the docs:
```bash
testerkit docs show concepts/configuration/parts
testerkit docs show concepts/configuration/capabilities
```
Sibling skills: `testerkit-tests` (guardbanding a characteristic into a test
limit — the other half of this skill's boundary), `testerkit-stations` (pin
