---
name: testerkit-capture
description: Use when a user wants to capture non-tabular test evidence during a test with TesterKit — record a waveform, a live sensor feed, a photo, a vendor capture file, or a log. This is the write side; reading any of it back afterwards is testerkit-data.
---

# Capturing evidence (channels + files)

Picking the capture **verb** (`observe` vs `stream` vs a file write) is a
`testerkit-tests` decision — see its verb table. This skill owns what happens
once you've picked a store: the write mechanics. Reading any of it back
afterwards is `testerkit-data`.
...
| a photo, vendor blob, structured report — one shot | FileStore | `observe(name, value)` or `testerkit.files.write(...)` |
| a growing file (DAQ capture, jsonl log, vendor format) | FileStore | `testerkit.files.stream(name, format=...)` |
...
```python
uri = testerkit.files.write("vendor_capture", vendor_blob)
```
...
`testerkit.files.write`/`.stream` require a `session_id` (auto-resolved from
the active session); `run_id` only lands in the sidecar when written
through these calls, not through `observe`'s auto-routing.

## 4. Reading it back → `testerkit-data`

Pulling a channel, resolving an artifact, or wiring a capture's `file://` URI
to a run is a read concern — see `testerkit-data`. There is no `testerkit
channels`/`files` CLI subcommand; readback is UI, MCP, HTTP, or the
`testerkit.channels`/`testerkit.files` modules.

## Best-practice defaults

- **Shape decides the store** — don't reach for `testerkit.files.write` on an
...
Read the docs:
```bash
testerkit docs show how-to/data/capture-waveform
testerkit docs show how-to/data/capture-an-artifact
testerkit docs show how-to/data/stream-live-channel
testerkit docs show concepts/data/three-verbs
```
Sibling skills: `testerkit-tests` (verb choice), `testerkit-data` (reading any of
this back), `testerkit-debug` (events around a capture).
