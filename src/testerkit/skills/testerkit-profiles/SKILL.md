...
`testerkit-tests`), plus profile-only `description` / `facets` / `extends`:
...
top of the stack, not a peer. See `testerkit-mocks` for the same rule
applied to `mocks:`.
...
testerkit validate                 # profiles/*.yaml against ProfileConfig
```

A profile listing a `tests:` key that matches no collected test is a
silent no-op on that key — TesterKit warns loudly at session start rather
...
Read the docs:
```bash
testerkit docs show how-to/execution/profiles
```
Sibling skills: `testerkit-tests` (sidecar layer, verb choice), `testerkit-mocks`
(per-test mock cascade), `testerkit-stations` (station types), `testerkit-sites`
