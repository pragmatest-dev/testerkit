...
per role (`psu`, `dmm`, `scope`, ...). TesterKit ships **no drivers**. Every
...
`verify`/`observe` test needs no instrument at all (`testerkit-tests`).
...
| spec what the DUT itself measures/tolerates | `testerkit-parts` |
| swap real instruments for canned returns at run time | `--mock-instruments` / `testerkit-mocks` |
...
`resource:` is a VISA string, TesterKit opens it as a raw PyVISA resource instead.

## 3. Discover what's connected

```bash
testerkit discover              # scan VISA/NI/serial/LXI, print *IDN? results
testerkit discover --visa --json
testerkit station init          # interactive: discovers, then prompts for a role per instrument
```

`testerkit discover` is read-only. `testerkit station init` walks each discovered
...
(`testerkit-parts`) before the fixture. `part_id` scopes the fixture to one
...
stations (MCP `testerkit_match(part_id=...)`).
...
are `testerkit-mocks`, not this skill.

## 7. Save, validate, run

```bash
testerkit validate stations/bench_01.yaml   # or: testerkit validate (scans stations/, parts/, fixtures/, catalog/)
...
```python
testerkit_schema(yaml_type="station")
testerkit_project(action="save", type="station", id="bench_01", content={...}, project=project_root)
```
...
Read the docs:
```bash
testerkit docs show how-to/configuration/configuring-stations
testerkit docs show how-to/configuration/custom-drivers
testerkit docs show concepts/configuration/stations
testerkit docs show concepts/configuration/fixtures
```
Sibling skills: `testerkit-tests` (verbs, right-sizing), `testerkit-parts` (DUT pins +
characteristics), `testerkit-mocks` (per-test mock overrides), `testerkit-sites`
