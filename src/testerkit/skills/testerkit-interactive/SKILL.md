...
Declare each prompt with the `testerkit_prompts` marker, keyed by name; read the
...
exactly one `testerkit_prompts` entry is in scope for that test; pass the key
...
handler wins, then `TESTERKIT_AUTO_CONFIRM=1`, then a tty fallback, else
`PromptUnavailableError`. **Set `TESTERKIT_AUTO_CONFIRM=1` for CI** so prompts
...
cascades into the marker like any other TesterKit field (right-sizing lives in
`testerkit-tests`).

**Session-level prompts are a different thing.** `required_inputs:` in
`testerkit.yaml` asks once, at session start, before any test runs — for a
...
(`TESTERKIT_SERIAL_NUMBER`) → the declared prompt. Unresolved after all three
fails the session before any test collects. Don't reach for this to gate a
single test — that's `testerkit_prompts`.
...
part of the `testerkit_prompts`/`required_inputs` path; don't reach for it in
new tests.

Depth: `testerkit docs show how-to/execution/operator-prompts` — the three
prompt types, marker-scope rules, and message-wording guidance in full.
Hung/timed-out prompts in CI are a `testerkit-debug` triage case
(`PromptUnavailableError`, `TESTERKIT_AUTO_CONFIRM` unset).
...
TesterKit's live data (channel samples, session/instrument events) arrives on a
background gRPC Flight reader thread, not the UI loop. `testerkit.ui.channel_data(id)`
...
`src/testerkit/ui/components/channel_values.py`. Full recipe + the
...
Depth: `testerkit docs show how-to/execution/custom-operator-ui` — full
step-by-step including reading channel data outside a page context.

## 3. Standalone interactive bench control (no pytest)

`testerkit.connect(station, mock=?)` returns a `StationConnection` for scripts,
notebooks, and the operator UI itself:

```python
import testerkit

with testerkit.connect("cell-7", mock=True) as station:
...
Live monitoring outside a page uses the module-level `testerkit.channels`
...
- **Per-test pause → `testerkit_prompts`/`prompt`; session-start ask →
  `required_inputs`.** Don't use one for the other's job.
- **`TESTERKIT_AUTO_CONFIRM=1` in every CI path** that touches a prompted test.
- **Only the event loop touches a NiceGUI element** — subscribe-and-mutate
  for one element, holder-and-`ui.timer` for many.
- **`channel_data(...).subscribe(cb)` is pre-marshaled; `testerkit.channels.live/
...
anti-pattern catalog for building NiceGUI pages against live TesterKit data.
Sibling skills: `testerkit-tests` (verb choice, `context` fixture),
`testerkit-capture` (writing to the channel/file stores), `testerkit-data`
(reading channels/files back), `testerkit-debug` (hung/timed-out prompt
triage), `testerkit-stations` (station
