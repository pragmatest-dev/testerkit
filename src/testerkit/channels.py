...
``stream`` pytest fixture; see :mod:`testerkit.pytest_plugin`). Power
...
:func:`testerkit.execution._state.get_channel_store`. The channel kind
...
from testerkit.data.channels import flight_manager
from testerkit.data.channels.client import ChannelClient, channel_query_client
...
def _resolve_store() -> Any:
    ...
    Raises if no store is wired — call from inside an active TesterKit
...
def write(name: str, sample: Any, *, namespace: str | None = None) -> str:
    ...
    Args:
        ...
        sample: One sample to append. Same shape rules as
            ...
            :func:`testerkit.data.ref.classify_value`. Per Position 2 +
...
class _ChannelSink:
    ...
    # Thresholds come from the store's ``ChannelOptions`` (testerkit.yaml channels:).
...
def stream(name: str, *, namespace: str | None = None) -> Iterator[_ChannelSink]:
    ...
    :func:`testerkit.filestore.stream` (signature today, real sink in
    build item 2 / C5).
