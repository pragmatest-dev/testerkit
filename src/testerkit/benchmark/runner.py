...
verdict — does TesterKit keep up with a busy test station? — and a curated
...
from testerkit.benchmark.core import (
    BenchContext,
    BenchmarkReport,
    WorkloadResult,
    time_workload,
)
from testerkit.benchmark.scenario import (
    PROFILES,
    ConcurrencyPoint,
    ConcurrencySweep,
    StorageFootprint,
...
from testerkit.benchmark.system import (
    ResourceSampler,
    collect_hardware,
    collect_versions,
    have_psutil,
...
def _measure_storage(data_dir: Path) -> StorageFootprint | None:
    ...
    from testerkit.benchmark.workloads import build_run, make_measurement
...
def run_benchmark(
    ...
    *,
    ...
    data_dir = Path(tempfile.mkdtemp(prefix="testerkit-bench-"))
...
# Rendering
#
# The human report answers ONE question — does TesterKit keep up with a test
...
def _verdict(report: BenchmarkReport) -> str:
    ...
    return (
        ...
        "TesterKit stays out of your test's way."
    )
...
def format_markdown(report: BenchmarkReport) -> str:
    out: list[str] = ["# TesterKit performance on this machine", ""]
...
def format_summary(report: BenchmarkReport) -> str:
    ...
    lines: list[str] = ["TesterKit performance on this machine", ""]
