...
project ``testerkit.yaml`` before any env var, so the only reliable way to
...
def _runs_worker(data_dir: str, scale: int, seed: int) -> tuple[float, float]:
    ...
    from testerkit.benchmark.workloads import build_run
...
def _representative_worker(data_dir: str, scale: int, seed: int) -> tuple[float, float]:
    ...
    """
    from testerkit.benchmark.scenario import PROFILES
