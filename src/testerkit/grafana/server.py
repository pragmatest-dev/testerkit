...
Exposes all TesterKit data stores (runs/parquet, events/IPC, channels/IPC)
...
Usage (standalone):
    python -m testerkit.grafana.server [--data-dir PATH] [--port 5433]
"""
...
def create_connection(data_dir: Path) -> duckdb.DuckDBPyConnection:
    """Create an in-memory DuckDB connection with VIEWs over TesterKit data.
...
def serve(
    ...
    *,
            ...
            try:
                ...
                if consecutive_failures >= 5:
                    _log.error(
                        ...
                        "Grafana may see stale data. Restart with: testerkit grafana serve",
    ...
    print(f"TesterKit pgwire server listening on {host}:{port}")
