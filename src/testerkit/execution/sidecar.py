...
def _merge_marker_fields_into(target: TestEntry, src: TestEntry) -> None:
    """In-place merge ``src``'s TesterKit-marker fields into ``target`` (last-wins).
...
def _resolve_single(
    ...
    * Anything else (no policy declared) → ``None`` so the measurement
    ...
    """
    from testerkit.execution.limits import _apply_guardband
    from testerkit.models.enums import Comparator
    from testerkit.models.test_config import Limit as LimitModel
...
def resolve_limit(
    ...
    *,
    ...
    """
    from testerkit.models.capability import SpecBand, band_matches
