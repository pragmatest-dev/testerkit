...
* **Profile resolution.** Load ``testerkit.yaml``, auto-synthesize a
...
from testerkit.execution.sidecar import _merge_entry_into
...
class PytestRunner(BaseModel):
    ...
    Tier 2 of the two-tier validation pattern: TesterKit core stores the
...
def load_project_defaults() -> ProjectConfig:
    """Load :class:`ProjectConfig` from ``testerkit.yaml``, falling back to defaults."""
    try:
        from testerkit.store import load_project_config

        return load_project_config()
    except Exception:  # noqa: BLE001 — any load failure falls back to defaults
        # Bad or missing testerkit.yaml — don't crash pytest over config
...
def validate_phase_wiring(
    ...
    1. **Station compliance.** If the active station declares
       ...
       :func:`testerkit.models.station.validate_station_against_type`.
    ...
    """
    from testerkit.models.station import validate_station_against_type
...
def required_input_key_to_env_var(key: str) -> str:
    ...
    Example: ``serial_number`` → ``TESTERKIT_SERIAL_NUMBER``.
    """
    return f"TESTERKIT_{key.upper()}"
...
def resolve_required_inputs(
    ...
    3. Operator prompt via :func:`testerkit.prompts.ask`, using the
...
def resolve_test_phase(requested_phase: str | None, mocks_active: bool = False) -> str:
    ...
    * ``--mock-instruments`` active — measurements aren't real.
    ...
    # ``testerkit.execution._git.is_git_clean`` — top-level binding here
    # would freeze the reference at import time and break the patch.
    from testerkit.execution._git import is_git_clean
