...
from testerkit.data.events import SyncArrived, SyncRelease
...
def get_sync(event_store: EventStore | None = None) -> SyncPoint | None:
    ...
    Returns None if not in a multi-site worker process (i.e., _TESTERKIT_SITE_INDEX
    ...
    session_id_str = os.environ.get("_TESTERKIT_SESSION_ID")
    session_id = UUID(session_id_str) if session_id_str else uuid4()

    if event_store is None:
        raise ValueError(
            "EventStore required for multi-site sync. "
            "_TESTERKIT_SITE_INDEX is set but no EventStore was provided."
        )
