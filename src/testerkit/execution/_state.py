...
from testerkit.data.models import CollectedItem
...
def get_active_test_characteristics() -> list[str]:
    ...
    ``testerkit_characteristics`` marker (or, when the marker is absent, the
...
def get_active_vector_params() -> dict[str, Any]:
    ...
    ``_testerkit_push_params`` at test start so ``RunScope.measure``
...
def get_current_site_index() -> int:
    ...
    In multi-site worker children, set from the ``_TESTERKIT_SITE_INDEX`` env
...
def resolve_session_id(
    ...
    *,
    ...
    inheritance (the ``testerkit.files.write`` user-facing surface, for
...
def no_active_resource_error(resource: str, *, explicit_arg: str = "") -> RuntimeError:
    ...
    Args:
        resource: Human label for the missing resource — e.g.
            ``"ChannelStore"``, ``"session_id"``, ``"TesterKit context"``.
