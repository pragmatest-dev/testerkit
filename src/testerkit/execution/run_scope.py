...
def _get_run_id() -> UUID:
    ...
    env_id = os.environ.get("_TESTERKIT_RUN_ID")
    if env_id:
...
class RunScope:
    ...
    def __init__(
        ...
        if git_commit is None or git_branch is None or git_remote is None or project_name is None:
            from testerkit.execution._git import get_git_info, get_project_name
    ...
    def log_measurement(self, measurement: Measurement):
        ...
        if measurement.limit_low is not None or measurement.limit_high is not None:
            try:
                from testerkit.pytest_plugin.hooks import mark_step_judgment_intent
    ...
    def measure(
        ...
        *,
        ...
        judgment"). :func:`testerkit.execution.verify.verify` resolves
    ...
    def _guard_duplicate(self, name: str, allow_repeat: bool) -> None:
        ...
        * **Vector index.** In-test iteration via the ``vectors``
          ...
          iteration.
        * **Vector retry.** :class:`~testerkit.execution.harness.TestHarness`
          retries (``testerkit_retry`` markers running through the
    ...
    def finalize(self) -> TestRun:
        ...
        (``testerkit_retry`` / ``pytest-rerunfailures`` reruns) collapse
        ...
        from testerkit.data.models import retry_aware_rollup
