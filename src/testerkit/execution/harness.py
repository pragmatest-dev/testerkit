...
from testerkit.data.ref import Latchable, classify_value, is_ref
...
from testerkit.execution.run_scope import _auto_traceability
from testerkit.execution.vectors import Vector, expand_vectors
from testerkit.execution.verify import _perform_measure, _perform_verify
from testerkit.models.test_config import Limit, MeasurementLimitConfig, PromptConfig, RetryConfig
from testerkit.prompts import ask
...
class LimitsView(Mapping[str, MeasurementLimitConfig]):
    """Dict-like view over the merged ``testerkit_limits`` for the active test.
...
class Context:
    ...
    def __init__(
        ...
        # ``testerkit.execution._state.resolve_session_id`` so harness /
        ...
        # is in ``testerkit.files.py:_resolve_session_id``, where
        ...
        # test declares via ``testerkit_characteristics`` / ``testerkit_connections``
        # markers. Populated by the pytest-native plugin's
        # ``_testerkit_resolve_connections`` autouse fixture; ``None`` when
    ...
    def observe(
        ...
        - **Sink handle** (``_ChannelSink`` from
          :func:`testerkit.channels.stream`, ``_BaseSink`` from
          :func:`testerkit.files.stream`, or any object satisfying the
          :class:`~testerkit.data.ref.Latchable` Protocol) → ``.uri``
    ...
    def _emit_vector_started(self) -> None:
        ...
        the counter survives pytest reruns, it counts in-body ``testerkit_retry``
        ...
        *,
        ...
        if self._channel_store is None:
            raise RuntimeError(
                ...
                "or run inside an active TesterKit session."
            )
    ...
    def characteristics(self) -> tuple[str, ...]:
        ...
        Sourced from the ``testerkit_characteristics`` marker (or the union of
    ...
    def limits(self) -> LimitsView:
        """Dict-like view over the merged ``testerkit_limits`` for this test.
...
class TestHarness:
    ...
    def step(self, name: str | None = None, description: str | None = None) -> Iterator[TestStep]:
        ...
        try:
            ...
            # collapse in retry_aware_rollup (testerkit.data.models), which
