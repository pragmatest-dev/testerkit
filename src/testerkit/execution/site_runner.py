...
if TYPE_CHECKING:
    ...
    from testerkit.execution.sync import SyncCoordinator
...
def _build_site_env(
    ...
    - ``_TESTERKIT_SITE_INDEX`` — which site index this process handles
    - ``TESTERKIT_UUT_SERIAL`` — UUT serial for this site
    - ``TESTERKIT_UUT_PART_NUMBER`` — optional UUT part number
    - ``TESTERKIT_UUT_REVISION`` — optional UUT revision
    - ``TESTERKIT_UUT_LOT_NUMBER`` — optional UUT lot/batch number

    The worker re-derives its own site's connections from the loaded
    fixture YAML by its resolved ``_TESTERKIT_SITE_INDEX`` (see the
    ``fixture_config`` fixture) — no serialized site config needs to
    ride the environment.
    """
    env = base_env.copy()
    env["_TESTERKIT_SITE_INDEX"] = str(site_index)
    env["TESTERKIT_UUT_SERIAL"] = uut.serial
    if uut.part_number:
        env["TESTERKIT_UUT_PART_NUMBER"] = uut.part_number
    if uut.revision:
        env["TESTERKIT_UUT_REVISION"] = uut.revision
    if uut.lot_number:
        env["TESTERKIT_UUT_LOT_NUMBER"] = uut.lot_number
    if site.uut_resource:
        env["TESTERKIT_UUT_RESOURCE"] = site.uut_resource
    return env
...
class SiteRunner:
    ...
    - ``_TESTERKIT_SESSION_ID`` — shared across all sites
    - ``_TESTERKIT_SITE_INDEX`` — which site index this process handles
    - ``_TESTERKIT_SITE_COUNT`` — total site count (for sync)
    - ``TESTERKIT_UUT_SERIAL`` — UUT serial for this site
    - ``TESTERKIT_UUT_PART_NUMBER`` — optional UUT metadata
    - ``TESTERKIT_UUT_RESOURCE`` — UUT driver connection string
    - ``_TESTERKIT_INSTRUMENT_SERVER`` — instrument server address (if shared instruments)
    - ``_TESTERKIT_SHARED_ROLES`` — comma-separated roles served remotely
    ...
    def run(  # noqa: PLR0912
        ...
        *,
        ...
        # Set shared env vars
        base_env["_TESTERKIT_SESSION_ID"] = str(self._session_id)
        base_env["_TESTERKIT_SITE_COUNT"] = str(len(self._sites))

        # Instrument server env vars for shared instruments
        if self._instrument_server_address and self._shared_roles:
            base_env["_TESTERKIT_INSTRUMENT_SERVER"] = self._instrument_server_address
            base_env["_TESTERKIT_SHARED_ROLES"] = ",".join(
        ...
        try:
            ...
            for site in self._sites:
                ...
                if event_log is not None:
                    from testerkit.data.events import SiteStarted
                ...
                t = threading.Thread(
                    ...
                    name=f"testerkit-site-{site.site_index}",
                    daemon=True,
                )
                t.start()
                threads.append(t)
            ...
            if event_log is not None:
                from testerkit.data.events import SiteCompleted
...
def is_orchestrator_mode(config) -> bool:
    ...
    1. ``_TESTERKIT_SITE_INDEX`` is NOT set (we're not a worker child)
    ...
    """
    if os.environ.get("_TESTERKIT_SITE_INDEX") is not None:
    ...
    from testerkit.pytest_plugin.helpers import find_fixture_file
...
def is_worker_mode() -> bool:
    ...
    return os.environ.get("_TESTERKIT_SITE_INDEX") is not None
...
def _run_subprocess_mode(
    ...
    from testerkit.execution.session_scope import open_session
    ...
    if shared_roles:
        from testerkit.instruments.lifecycle import disconnect, load_and_connect
        from testerkit.instruments.server import InstrumentServer
    ...
    try:
        ...
        from testerkit.execution._state import set_active_site_runner
        ...
        from testerkit.instruments.lifecycle import disconnect
...
def run_multi_site_session(
    ...
    from testerkit.execution._state import get_current_run_scope
    from testerkit.execution.sites import detect_shared_instruments, resolve_fixture_sites
    from testerkit.execution.uut_provider import CLIUUTProvider
    from testerkit.pytest_plugin import _mocks_active
    from testerkit.pytest_plugin.helpers import find_fixture_file
    from testerkit.store import load_fixture, load_project_config
