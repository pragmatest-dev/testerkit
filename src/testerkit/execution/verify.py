...
:mod:`testerkit.execution._state`/:mod:`logger` and exposed through
...
from testerkit.data.models import Measurement, Outcome
from testerkit.models.test_config import Limit, coerce_limit
...
class _LimitsMapping(Mapping[str, Limit]):
    ...
    def __getitem__(self, key: str) -> Limit:
        from testerkit.execution._state import get_active_characteristic
        from testerkit.execution.sidecar import resolve_limit
...
def _perform_verify(
    ...
    * ``observe`` handles **all shapes** — scalar / array / Waveform /
    ...
    from testerkit.execution.run_scope import _resolve_measurement_limit

    run_scope = get_current_run_scope()
    if run_scope is None:
        raise RuntimeError(
            "verify() called without an active TesterKit run scope — "
            "is a TesterKit runner plugin installed?"
        )
    ...
    with char_ctx:
        ...
        if effective_limit is None:
            ...
            from testerkit.execution._state import get_active_profile
            ...
            raise MissingLimitError(
                ...
                "@pytest.mark.testerkit_limits / sidecar / profile / part spec, "
...
def build_verify_callable() -> VerifyFn:
    ...
    primitive (e.g. ``@pytest.fixture`` in :mod:`testerkit.pytest_plugin`).
...
def _perform_measure(
    ...
    if run_scope is None:
        raise RuntimeError(
            "measure() called without an active TesterKit run scope — "
            "is a TesterKit runner plugin installed?"
        )
