...
from testerkit.cli._common import _get_data_dir
...
def data_prune(
    ...
    """
    from testerkit.data.retention import prune_all
...
def _global_data_dir() -> Path:
    ...
    Mirrors the fallback in testerkit.data.data_dir.resolve_data_dir but
...
def data_promote(include_starter: bool, dry_run: bool, with_events: bool) -> None:
    ...
    Starter projects ship with ``data_dir: data`` in testerkit.yaml so
    ...
    `testerkit data promote` copies non-starter runs **plus the channel/file
    ...
    testerkit.yaml. ``--with-events`` also carries each run's session events.
    ...
    if not found:
        raise click.ClickException(
            "No testerkit.yaml found in this directory or any parent. "
            "`testerkit data promote` runs inside a project directory."
        )
    project_root, project = found

    if not project.data_dir:
        raise click.ClickException(
            "This project's testerkit.yaml has no `data_dir` override, "
    ...
    if dry_run:
        click.echo("\n[dry-run] No files copied. testerkit.yaml unchanged.")
        return

    if not to_copy:
        click.echo("\nNothing to promote. testerkit.yaml unchanged.")
        return
    ...
    # Update testerkit.yaml — drop the data_dir override so future runs
    # and queries from this project use the global store. Uses ruamel
    # to preserve formatting + comments.
    testerkit_yaml = project_root / "testerkit.yaml"
    yaml = YAML()
    yaml.preserve_quotes = True
    with testerkit_yaml.open() as f:
        doc = yaml.load(f)
    if "data_dir" in doc:
        del doc["data_dir"]
        with testerkit_yaml.open("w") as f:
            yaml.dump(doc, f)
        try:
            display_path = testerkit_yaml.relative_to(Path.cwd())
        except ValueError:
            display_path = testerkit_yaml
        click.echo(f"Updated {display_path}: removed `data_dir` override.")

    click.echo("")
    click.echo(
        "Future runs and `testerkit runs` queries from this project now use the global store."
    )
    click.echo(
...
def data_reindex(data_dir: str | None) -> None:
    ...
    Use this when the index is out of date (e.g. after upgrading testerkit).
    """
    ...
    for subdir, mgr_cls in [
        ...
        if d.exists():
            ...
            # Glob covers every store's index files — a legacy single-file
            # ``_index.duckdb`` and the content-addressed epoch files
            # ``_index.<fp>.duckdb`` (runs + events, #53/#64) — force-drop every
            # epoch so the daemon rebuilds fresh.
...
# ── testerkit data index — derived-index (DuckDB) epoch lifecycle (#53 P4/P5) ──
#
...
def _format_bytes(n: int) -> str:
    """Human-readable byte count (B/KB/MB/GB) for ``testerkit data index`` output.

    No repo-wide byte-size helper is reusable here without pulling in
    ``testerkit.ui`` (NiceGUI) as a CLI-time import (``format_file_size`` in
    ``testerkit.ui.shared.components``) — this is a small local equivalent,
...
def old_epoch_hint(data_dir: str | None = None, *, min_bytes: int = _HINT_MIN_BYTES) -> str | None:
    ...
    *suggests* ``testerkit data index prune``, which is the ledger-aware actor that
    ...
    after ``testerkit setup``) must never break setup. Non-destructive.
    """
    ...
    try:
        ...
        return (
            ...
            "Reclaim unused ones: testerkit data index prune"
        )
...
def _read_epoch_meta_readonly(path: Path) -> tuple[dict[str, str], int | None] | None:
    ...
    from testerkit.data._runs_duckdb_daemon import _read_index_meta
...
def _read_epoch_meta_via_daemon(runs_dir: Path) -> tuple[dict[str, str], int | None]:
    ...
    ``acquire``) — so ``testerkit data index list`` can wake an idle daemon. That
    also drops the lock and lets a direct read-only open succeed next time;
    querying through Flight here works either way.
    """
    client = _flight_client_for_runs(runs_dir, "testerkit data index")
...
@dataclass
class _EpochRow:
    """One rendered row for ``testerkit data index list`` — display-only, never persisted."""
...
def data_index_list(data_dir: str | None) -> None:
    ...
    from testerkit.data._runs_duckdb_daemon import _projection_fingerprint, _read_epochs_ledger
    ...
    if not epoch_files:
        click.echo(
            "No index epochs yet — run `testerkit data index build` or any query to create one."
        )
        return
    ...
    for path in epoch_files:
        ...
        seen_by = ledger_entry.get("seen_by") or (
            [meta["testerkit_version"]] if meta.get("testerkit_version") else []
        )
        rows.append(
            _EpochRow(
                current=is_current,
                ...
                built_by=meta.get("testerkit_version", "?"),
...
def _poll_until_warm(
    ...
    *,
    ...
    by this version, e.g. a parquet from a NEWER testerkit version is *deferred*
...
def data_index_build(data_dir: str | None, rebuild: bool, background: bool) -> None:
    ...
    client = _flight_client_for_runs(runs_dir, "testerkit data index build")
    ...
    _testerkit_version, schema_version, _fp = _current_provenance()
    ...
    if unreconciled and not timed_out:
        click.echo(
            f"{unreconciled} file(s) not indexed — written by a newer testerkit "
...
def data_index_rm(fingerprint: str, data_dir: str | None, force: bool) -> None:
    ...
    if not matches:
        raise click.ClickException(
            ...
            "Run `testerkit data index list` to see what's available."
        )
    if len(matches) > 1:
    ...
    if fp12 == current_fp12 and not force:
        raise click.ClickException(
            ...
            "is restarted first), or use `testerkit data index build --rebuild` instead."
        )
...
def data_index_prune(data_dir: str | None, keep_last: int, older_than: str, dry_run: bool) -> None:
    ...
    Mirrors ``testerkit data prune`` (durable data) — same verb, index layer.
    """
    from testerkit.data._runs_duckdb_daemon import (
        _projection_fingerprint,
        _read_epochs_ledger,
    ...
    from testerkit.data.retention import parse_duration
...
def data_import(source: Path, data_dir: str | None) -> None:
    ...
    """
    from testerkit.data.channels.flight_manager import FlightDaemonManager
    from testerkit.data.duckdb_manager import DuckDBDaemonManager
