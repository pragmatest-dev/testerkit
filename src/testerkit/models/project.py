...
Schema for ``testerkit.yaml`` project config files — flat, all fields at root.
"""
...
from testerkit.models.data_options import ChannelOptions, FileOptions, SessionOptions, StreamTuning
from testerkit.models.test_config import PromptConfig, TestEntry
...
class ProfileConfig(TestEntry):
    ...
    ``runner:`` is a flat per-runner config block — opaque to TesterKit
...
class MultiSiteConfig(BaseModel):
    ...
    Surfaced as ``multi_site:`` in ``testerkit.yaml``; consumed by the
