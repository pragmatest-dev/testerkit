...
- These models hold the **producer-local** knobs — they run in the project's
  own process, so a project owns them and may set them in ``testerkit.yaml`` under
...
class ChannelOptions(BaseModel):
    """Channel-store data options (settable in ``testerkit.yaml`` under ``channels:``).
...
class StreamTuning(BaseModel):
    ...
    durable checkpoint (settable in ``testerkit.yaml`` under ``stream:``).
...
class FileOptions(BaseModel):
    """File-store data options (settable in ``testerkit.yaml`` under ``files:``)."""

    model_config = {"extra": "forbid", "frozen": True}

    # Blob-backend root URI for FileStore artifacts. Unset keeps blobs under
    # ``{data_dir}/files``; an ``s3://`` / ``gcs://`` URI stores them in an
    # object store — only this changes, no code does. (``TESTERKIT_FILES_BACKEND``
    # env overrides this.)
