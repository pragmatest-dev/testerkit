...
``testerkit.connect()`` is the main entry point for non-pytest instrument access.
...
Usage::
    ...
    with testerkit.connect("cell-7") as station:
        dmm = station.instrument("dmm")
        v = dmm.measure_voltage()

    # Or explicit lifecycle for UIs:
    station = testerkit.connect("cell-7")
    station.start()
    dmm = station.instrument("dmm")
...
from testerkit.data.events import InstrumentConfigure
from testerkit.execution._state import get_current_run_scope, get_current_step
from testerkit.execution.session_scope import SessionScope, build_session_started, open_session
from testerkit.instruments.pool import InstrumentPool
from testerkit.models.data_options import SessionOptions, StreamTuning
from testerkit.models.instrument import InstrumentRecord
from testerkit.models.station import StationConfig
from testerkit.signals import deregister_cleanup, register_cleanup
...
class StationConnection:
    ...
    def start(self) -> None:
        ...
        # Resolve the will from the project's session options (testerkit.yaml
        ...
        # project config (testerkit.yaml channels:), not the station config; absent →
        ...
        # module-level surfaces (``testerkit.channels.stream``, ``testerkit.files.write``)
    ...
    def sync(self, name: str, timeout: float | None = None) -> None:
        ...
        In single-site mode (no _TESTERKIT_SITE_INDEX), returns immediately.
...
def _find_project_config() -> tuple[Path, Any] | None:
    """Find testerkit.yaml in CWD ancestors. Returns (project_root, ProjectConfig) or None."""
    from testerkit.store import load_project

    current = Path.cwd()
    while current != current.parent:
        candidate = current / "testerkit.yaml"
        if candidate.exists():
            try:
...
def connect(
    ...
    *,
    ...
    Args:
        station: Station ID. If None, uses ``default_station`` from ``testerkit.yaml``.
        data_dir: Where to write events. Falls back to testerkit.yaml → TESTERKIT_HOME.
        mock: Use mock instruments.

    Returns:
        A ``StationConnection`` (usable as context manager or via start/stop).
    """
    from testerkit.store import find_station_config

    if station is None:
        station = _default_station_id()
        if station is None:
            raise ValueError(
                "No station specified and no default_station in testerkit.yaml. "
                "Pass a station ID: testerkit.connect('my-station')"
            )
...
def _default_station_id() -> str | None:
    """Read default_station from testerkit.yaml in CWD ancestors."""
