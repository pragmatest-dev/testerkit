...
def load_run_data(run_id: str, data_dir: str | None = None) -> ReportData:
    ...
    Args:
        ...
        data_dir: Data directory. Defaults to the project's resolved data
            dir (``testerkit.yaml`` ``data_dir``, else the global default).
...
def _fmt_dt_raw(val: Any) -> str:
    ...
    consumers (e.g. the terminal ``testerkit show`` summary) can re-render it.
...
def _resolve_template(template: str, template_dir: str | None) -> str:
    ...
    2. Built-in templates: testerkit/reports/templates/{template}.html
    """
    # Project templates
    project_paths = []
    if template_dir:
