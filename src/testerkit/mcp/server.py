...
All tools are prefixed ``testerkit_`` to namespace against other MCP
servers in a multi-server agent setup. Within the prefix:

- **Single-purpose actions** use ``testerkit_<verb>``:
  ``testerkit_discover`` (scan instruments), ``testerkit_match`` (check
  compatibility), ``testerkit_run`` (execute tests), ``testerkit_open``
  (browser URL), ``testerkit_schema`` (JSON Schema lookup).
- **Domain-scoped read tools** use ``testerkit_<noun>`` where the noun
  is the table or store being queried. Sub-actions ride on an
  ``action=`` parameter so the tool count stays manageable:
  ``testerkit_runs(action="list"|"get")``, ``testerkit_steps(action=
  "list"|"tree")``, ``testerkit_metrics(action="summary"|"pareto"|
  "ppk"|"trend"|"retest"|"time_loss")``. Single-action queries
  drop the ``action`` parameter: ``testerkit_events``, ``testerkit_
  sessions``, ``testerkit_channels``, ``testerkit_files``.
- **Project-scoped CRUD** is ``testerkit_project`` — the unified
...
- ``testerkit_project`` — Unified project CRUD (init, list, get,
  save, read, lookup_enum, enum_reference)
- ``testerkit_discover`` — Scan for VISA instruments
- ``testerkit_match`` — Check compatibility between parts /
  stations / fixtures
- ``testerkit_run`` — Execute tests and return results
- ``testerkit_open`` — Get URL to view/edit in browser
- ``testerkit_schema`` — Get JSON Schema for YAML validation
- ``testerkit_events`` — Query events from the event store
- ``testerkit_sessions`` — List known sessions
- ``testerkit_channels`` — Query channel data
- ``testerkit_files`` — List FileStore artifacts (blobs, waveforms, streams)
- ``testerkit_metrics`` — Query manufacturing-test analytics
- ``testerkit_runs`` — Query the runs summary table
- ``testerkit_steps`` — Query the steps table for one run
...
from testerkit.mcp.tools import (
    channels_tool,
    discover_tool,
    events_tool,
    files_tool,
...
from testerkit.schema_export import SCHEMA_MAP
...
def _build_instructions() -> str:
    ...
    return f"""\
TesterKit: Hardware test platform. Creates tests from datasheets.
...
```
1. Ask user where to create the project → testerkit_project(action="init", path="...")
2. testerkit_schema(yaml_type="part") → Get exact part schema
3. Extract specs from datasheet → Show to user → Ask approval → Save part
4. testerkit_schema(yaml_type="station") → Get exact station schema
5. testerkit_discover() → Show station config → Ask approval → Save station
6. Show test plan → Ask approval → Save BOTH test .py AND config.yaml
7. Confirm ready → testerkit_run() → Show results
```
...
**ALWAYS call `testerkit_schema(yaml_type=...)` before generating ANY YAML.**
...
- **MeasurementFunction** enum: `{", ".join(mf_values[:10])}`, ...
  (call `testerkit_schema(yaml_type="part")` for full list)
...
- `testerkit_project(action="init", path="~/project")` — Initialize, returns project_root
- `testerkit_project(action="save", type="part|station|test", id="...",
   content={{...}}, project=...)`
- `testerkit_project(action="read", path="template:test", project=...)` — Get templates
- `testerkit_schema(yaml_type="part|station|catalog|fixture")` — **Call FIRST**
- `testerkit_discover()` — Scan for connected instruments
- `testerkit_match(requirements=[...], project=...)` — Recommend catalog instruments
- `testerkit_run(test="tests/test_x.py", station="...", serial="...", project=...)`
- `testerkit_project(action="lookup_enum", id="FRES")` — Resolve datasheet abbreviation
- `testerkit_project(action="enum_reference")` — Full enum abbreviation table
- `testerkit_open(type="part|station|run", id="...")` — Get UI URL

## Key Rules

1. **STOP at each step** — Show plan, ask approval, wait for response
2. **Pass project=** to all calls after init
3. **testerkit_schema() before ANY save** — match the schema exactly
...
1. **Ask where to create the project** — suggest `~/testerkit-<part_number>` (user chooses).
   Then: `testerkit_project(action="init", path="<user's chosen path>")`
   - Returns `project_root` — USE THIS in all subsequent calls

2. **Get Part Schema**: `testerkit_schema(yaml_type="part")`
...
3. **Extract & Save Part Spec**: Parse datasheet, propose characteristics,
   ask approval, save with `testerkit_project(action="save", type="part", ...)`

4. **Get Station Schema**: `testerkit_schema(yaml_type="station")`
   - Run `testerkit_discover()` first. Use real addresses if instruments found,
...
6. **Create Test Files**: MUST create BOTH files
   ...
   ```python
   testerkit_project(action="save", type="test", id="tests/test_part.py", content={
...
7. **Run Tests**:
   ```python
   testerkit_run(test="tests/test_part.py", station="test_bench",
...
2. **Pass project=** to ALL calls after init
3. **testerkit_schema() before ANY save** — the schema is the ONLY source of truth
...
def create_mcp_server() -> FastMCP:
    """Create and configure the TesterKit MCP server."""
    mcp = FastMCP(
        "TesterKit",
    ...
    # -------------------------------------------------------------------------
    # Tool 1: testerkit_project (unified CRUD over project entities)
    # -------------------------------------------------------------------------

    @mcp.tool(name="testerkit_project")
    def testerkit_project(
        action: str,
        type: str | None = None,
        id: str | None = None,
        path: str | None = None,
        ...
        """Unified TesterKit operations: init, list, get, save, read.

        Actions:
        - init: Initialize project directory (returns project_root to use in subsequent calls)
          testerkit_project(action="init", path="~/my-project")

        - list: List entities of a type
          testerkit_project(action="list", type="part", project="/path/to/project")

        - get: Get entity details
          testerkit_project(action="get", type="part", id="tps54302", project="/path/to/project")
        ...
        - read: Read project file or template
          testerkit_project(action="read", path="parts/x.yaml", project="/path/to/project")
          testerkit_project(action="read", path="template:test", project="/path/to/project")

        - lookup_enum: Resolve datasheet abbreviations to enum values
          testerkit_project(action="lookup_enum", id="FRES") → resistance_4w
          testerkit_project(action="lookup_enum", id="Q") → [quality_factor, charge]

        - enum_reference: Get full abbreviation table as markdown
          testerkit_project(action="enum_reference")
        ...
        """
        return testerkit_tool(action, type, id, path, content, create, scaffold, project)

    # -------------------------------------------------------------------------
    # Tool 2: testerkit_discover
    # -------------------------------------------------------------------------

    @mcp.tool(name="testerkit_discover")
    ...
    # -------------------------------------------------------------------------
    # Tool 3: testerkit_match
    # -------------------------------------------------------------------------

    @mcp.tool(name="testerkit_match")
    def match(
    ...
    # -------------------------------------------------------------------------
    # Tool 4: testerkit_run
    # -------------------------------------------------------------------------

    @mcp.tool(name="testerkit_run")
    def run(test: str, station: str, serial: str, project: str) -> dict[str, Any]:
        ...
        Args:
            ...
            project: Project root path (from testerkit action='init' response)
    ...
    # -------------------------------------------------------------------------
    # Tool 5: testerkit_open
    # -------------------------------------------------------------------------

    @mcp.tool(name="testerkit_open")
    ...
    # -------------------------------------------------------------------------
    # Tool 6: testerkit_schema
    # -------------------------------------------------------------------------

    @mcp.tool(name="testerkit_schema")
    ...
    # -------------------------------------------------------------------------
    # Tool 7: testerkit_events
    # -------------------------------------------------------------------------

    @mcp.tool(name="testerkit_events")
    def query_events(
    ...
    # -------------------------------------------------------------------------
    # Tool 8: testerkit_sessions
    # -------------------------------------------------------------------------

    @mcp.tool(name="testerkit_sessions")
    ...
    # -------------------------------------------------------------------------
    # Tool 9: testerkit_channels
    # -------------------------------------------------------------------------

    @mcp.tool(name="testerkit_channels")
    def query_channels(
        channel_id: str,
        session_id: str | None = None,
    ...
    # -------------------------------------------------------------------------
    # Tool: testerkit_files
    # -------------------------------------------------------------------------

    @mcp.tool(name="testerkit_files")
    def query_files(
        uri: str | None = None,
        session_id: str | None = None,
    ...
    # -------------------------------------------------------------------------
    # Tool 10: testerkit_metrics
    # -------------------------------------------------------------------------

    @mcp.tool(name="testerkit_metrics")
    def query_metrics(
        action: str,
        part: str | None = None,
    ...
    # -------------------------------------------------------------------------
    # Tool 11: testerkit_runs
    # -------------------------------------------------------------------------

    @mcp.tool(name="testerkit_runs")
    def query_runs(
        action: str = "list",
        run_id: str | None = None,
        limit: int = 50,
    ...
    # -------------------------------------------------------------------------
    # Tool 12: testerkit_steps
    # -------------------------------------------------------------------------

    @mcp.tool(name="testerkit_steps")
    def query_steps(
        run_id: str,
        action: str = "list",
        project: str | None = None,
