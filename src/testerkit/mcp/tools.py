...
Tools:
- testerkit: Unified CRUD (init, list, get, save, read)
- testerkit_discover: Scan for VISA instruments
- testerkit_match: Check compatibility
- testerkit_run: Execute tests
- testerkit_open: Get browser URL
"""
...
from testerkit.utils.enum_meta import lookup_enum as _lookup_enum_fn
from testerkit.utils.enum_meta import render_enum_reference as _render_enum_reference_fn
...
def _lookup_enum(term: str) -> dict[str, Any]:
    ...
    if not term:
        return {
            ...
            "e.g. testerkit_project(action='lookup_enum', id='FRES')"
        }
...
def testerkit_tool(
    ...
    if not project:
        return {
            ...
            " - use the path from testerkit_project(action='init')"
        }
...
def _init_project(
    ...
    Uses shared scaffolding from testerkit.init for consistency with CLI.
    """
    from testerkit.init import get_project_contents, init_project
    ...
    return {
        ...
        "next_steps": [
            ...
            "Use testerkit_project(action='read', path='template:test') to see test pattern",
            "Create a part with testerkit_project(action='save', type='part', ...)",
        ],
    }
...
def _list_parts(project: str) -> list[dict[str, Any]]:
    ...
    from testerkit.store import list_parts
...
def _get_catalog_entry(entry_id: str, project: str) -> dict[str, Any]:
    ...
    from testerkit.ui.shared.services import load_catalog_entry_by_type
...
def _get_instrument_asset(instrument_id: str, project: str) -> dict[str, Any]:
    ...
    from testerkit.ui.shared.services import load_instrument_asset_by_id
...
def _get_run(run_id: str, project: str) -> dict[str, Any]:
    ...
    from testerkit.api.schemas import load_run_view
...
def _validate_against_schema(
    ...
    from testerkit.schema_export import SCHEMA_MAP, FileType
...
def _save_entity(
    ...
    if entity_type == "station":
        ...
        from testerkit.store import normalize_and_check_instrument_types, save_station
    ...
    if entity_type == "part":
        ...
        from testerkit.store import save_part
    ...
    if entity_type == "fixture":
        ...
        from testerkit.store import save_fixture
    ...
    if entity_type == "catalog":
        ...
        from testerkit.store import save_catalog_entry
    ...
    if entity_type == "instrument_asset":
        ...
        from testerkit.store import save_instrument_asset
...
``context`` and ``verify`` fixtures come from the TesterKit pytest plugin.
"""
...
def discover_tool(protocols: list[str] | None = None) -> dict[str, Any]:
    ...
    """
    try:
        from testerkit.instruments.discovery import discover_and_identify, list_protocols
...
def run_tool(test: str, station: str, serial: str, project: str | None = None) -> dict[str, Any]:
    ...
    if not project:
        return {
            ...
            " - pass the path returned from testerkit_project(action='init')"
        }
...
def _resolve_data_dir(project: str | None) -> Path | None:
    """Resolve a project's data dir, honoring its ``testerkit.yaml`` ``data_dir``.
...
def files_query(
    ...
    *,
    ...
    """
    from testerkit.data.data_dir import resolve_data_dir
    from testerkit.data.files import catalog_manager
...
def schema_tool(yaml_type: str | None = None) -> dict[str, Any]:
    """Get JSON Schema for TesterKit YAML file types.
...
# ---------------------------------------------------------------------------
# Tool 10: testerkit_metrics — manufacturing-test analytics
# ---------------------------------------------------------------------------
