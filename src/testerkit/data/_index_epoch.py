...
def stamp_index_meta(
    ...
    *,
    ...
    ``(testerkit_version, schema_version, fingerprint)`` triple.
    """
    ...
    conn.execute(
        ...
        "('testerkit_version', ?), ('schema_version', ?), ('projection_fingerprint', ?) "
        "ON CONFLICT (key) DO UPDATE SET value = excluded.value",
        [testerkit_version, schema_version, fingerprint],
    )
    conn.execute(
...
def stamp_epochs_ledger(store_dir: Path, fingerprint: str, testerkit_version: str) -> None:
    ...
    try:
        ...
        if isinstance(prior, dict):
            ...
            if isinstance(prior_seen_by, list):
                ...
                legacy_version = prior.get("testerkit_version")
                if legacy_version:
                    seen_by = [str(legacy_version)]
        seen_by = sorted({*seen_by, testerkit_version})
        data[key] = {
            "seen_by": seen_by,
...
def read_epochs_ledger(store_dir: Path) -> dict[str, dict[str, Any]]:
    ...
    Used by ``testerkit data index list``/``gc`` (§7) to source SEEN BY / LAST
    SEEN without opening every epoch file. Tolerates the pre-P5 ledger shape
    (``{fp12: {testerkit_version, last_seen}}``) by folding a legacy
    ``testerkit_version`` into a single-element ``seen_by`` — mirrors the
    ...
    for fp12, entry in raw.items():
        ...
        if not isinstance(seen_by, list):
            legacy_version = entry.get("testerkit_version")
...
def remove_epochs_ledger_entries(store_dir: Path, fp12s: set[str]) -> None:
    ...
    Used by ``testerkit data index rm``/``gc`` (§7) after unlinking an epoch
