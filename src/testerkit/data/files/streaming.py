...
- :func:`testerkit.files.stream` — top-level power-user verb
...
:mod:`testerkit.data.files.serializers` registry. Each format defines
...
from testerkit.data.events import FileCheckpoint, FileEnded, FileStarted
from testerkit.data.files.catalog_manager import open_frame_relay

# Optional-extra deps: nptdms (TDMS) and h5py (HDF5) are gated behind
# install extras (``pip install testerkit[tdms,hdf5]``). The top-level
...
class StreamFormat:
    ...
    Mirrors :class:`testerkit.data.files.serializers.Serializer` but for
...
def get_format(name: str) -> StreamFormat:
    ...
    try:
        ...
        raise ValueError(
            f"testerkit.files.stream: unknown format {name!r}. "
            f"Known formats: {known}. Register a custom format with "
            "testerkit.data.files.streaming.register_format(name, fmt)."
        ) from None
...
class _BaseSink:
    ...
    def uri(self) -> str:
        ...
        Satisfies the :class:`~testerkit.data.ref.Latchable` protocol —
        :meth:`Context.observe` checks for this property and stamps
        the URI without re-writing when handed a sink:

        ::

            with testerkit.files.stream("capture", format="raw") as sink:
                sink.write(chunk)
    ...
    def _session_uuid(self) -> UUID:
        ...
        try:
            ...
            return uuid5(NAMESPACE_OID, f"testerkit-session-stub:{self._session_id_str}")
...
class _RawByteSink(_BaseSink):
    ...
    def write(self, chunk: Any) -> int:
        ...
        if isinstance(chunk, str):
            raise TypeError(
                "testerkit.files.stream(format='raw'): str chunk rejected. "
        ...
        if not isinstance(chunk, bytes | bytearray | memoryview):
            raise TypeError(
                f"testerkit.files.stream(format='raw'): chunk must be bytes/"
...
class _TdmsSink(_BaseSink):
    ...
    def __init__(
        ...
        *,
        ...
        if not _HAS_TDMS:
            raise ImportError(
                "testerkit.files.stream(format='tdms') requires the `npTDMS` "
                "package. Install with: pip install testerkit[tdms]"
            )
    ...
    def write(self, chunk: Any) -> int:
        ...
        for obj in objects:
            if not isinstance(obj, ChannelObject):
                raise TypeError(
                    "testerkit.files.stream(format='tdms'): chunks must be "
...
class _H5Sink(_BaseSink):
    ...
    def __init__(
        ...
        *,
        ...
        if not _HAS_HDF5:
            raise ImportError(
                "testerkit.files.stream(format='h5') requires the `h5py` "
                "package. Install with: pip install testerkit[hdf5]"
            )
    ...
    def write(self, chunk: Any) -> int:
        ...
        if not isinstance(chunk, dict):
            raise TypeError(
                "testerkit.files.stream(format='h5'): chunk must be a dict "
