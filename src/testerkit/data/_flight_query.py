...
MeasurementsQuery — every read path that talks to a testerkit daemon
over Flight.

Two responsibilities, both shared across the site:

1. **Channel pooling**: one ``flight.FlightClient`` per location
   per process. ``testerkit serve`` constructs fresh
...
2. **Selective retry**: errors are classified via
   :mod:`testerkit.data._flight_errors` and routed through
   :func:`testerkit.data._flight_retry.with_retry`. Transient
...
from testerkit.data._flight_retry import with_retry
...
class FlightQueryClient:
    ...
    def query(self, sql: str) -> list[dict[str, Any]]:
        ...
        :mod:`testerkit.data._flight_errors` for the classification.
        """
    ...
    def close(self) -> None:
        ...
        made ``testerkit serve`` abort. The pool is released only at
