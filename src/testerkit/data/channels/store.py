...
from testerkit.data._ipc_writer import BufferedIPCWriter
from testerkit.data._push_relay import PushRelay
from testerkit.data.channels import flight_manager
from testerkit.data.channels.index import ChannelIndex, _decimate_table, _to_utc
...
class ChannelStore:
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        # Default on; TESTERKIT_CHANNELS_SYNC_PUSH=1 forces the inline sync push
        # (A/B benchmarking + rollback).
        self._async_push = os.environ.get("TESTERKIT_CHANNELS_SYNC_PUSH") != "1"
    ...
    def _ensure_writer(
        ...
        schema = _infer_schema(self._normalize_value(first_value, sample_interval)).with_metadata(
            {
                b"testerkit.channel_descriptor": self._registry[channel_id]
                .model_dump_json()
                .encode(),
            ...
            if writer is None:
                ...
                if desc_obj is not None:
                    put_schema = put_schema.with_metadata(
                        {b"testerkit.channel_descriptor": desc_obj.model_dump_json().encode()}
                    )
