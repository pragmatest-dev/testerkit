...
1. An explicit handler installed via :func:`set_prompt_handler` (e.g.
   ``testerkit.api.dialogs.register_as_prompt_handler()`` when the UI is
   active — routes prompts through the dialog queue + UI).
2. ``TESTERKIT_AUTO_CONFIRM`` truthy — non-interactive auto-resolution for
...
TESTERKIT_AUTO_CONFIRM = "TESTERKIT_AUTO_CONFIRM"
...
def ask(config: PromptConfig) -> Any:
    ...
    if os.environ.get(TESTERKIT_AUTO_CONFIRM):
    ...
    raise PromptUnavailableError(
        ...
        "stdin is not a tty, and TESTERKIT_AUTO_CONFIRM is unset."
    )
...
def select_value(
    ...
    *,
    ...
    :mod:`testerkit.api.dialogs.manager` end with the same prompt_type
