...
:mod:`testerkit.pytest_plugin.hooks` (pytest_* lifecycle),
:mod:`testerkit.pytest_plugin.autouse` (autouse fixtures), and the
session fixtures defined in :mod:`testerkit.pytest_plugin.__init__`.
"""
...
from testerkit.execution.profiles import load_project_defaults
from testerkit.execution.site_runner import is_worker_mode
...
def mocks_active(config: pytest.Config) -> bool:
    ...
    2. Env var ``TESTERKIT_MOCK_INSTRUMENTS=1`` — set by the API runner so
    ...
    if cli is not None:
        return bool(cli)
    env = os.environ.get("TESTERKIT_MOCK_INSTRUMENTS")
    if env is not None:
        return env == "1"
...
def is_multi_site_worker() -> bool:
    ...
    return is_worker_mode() and int(os.environ.get("_TESTERKIT_SITE_COUNT", "1")) > 1
...
def prompt_for_serial(test_phase: str, site_index: int | None = None) -> str:
    ...
    if sys.stdin.isatty():
        serial = input(
            f"[testerkit] test_phase='{test_phase}' requires a UUT serial{label}.\n"
