...
params into the shared :class:`Context`, merging TesterKit-marker payloads
...
:mod:`testerkit.pytest_plugin.__init__`.
"""
...
from testerkit.execution.connections import (
    ConnectionIterator,
    ConnectionResolutionError,
    resolve_test_connections,
)
from testerkit.execution.harness import Context
from testerkit.execution.mocks import install_mocks
from testerkit.execution.run_scope import RunScope
from testerkit.models.test_config import MeasurementLimitConfig, MockEntry
from testerkit.pytest_plugin.helpers import safe_get_session_fixture
...
def _extract_characteristic_marker_ids(node: pytest.Item) -> list[str]:
    """Pytest adapter — collect ``testerkit_characteristics`` payloads.
    ...
    """
    marker = next(iter(node.iter_markers("testerkit_characteristics")), None)
    if marker is None:
        return []
    if marker.kwargs:
        raise pytest.UsageError(
            "testerkit_characteristics does not accept keyword arguments; pass "
...
def _scope_characteristics(
    ...
    if marker_ids:
        ...
        if unlisted:
            raise pytest.UsageError(
                f"testerkit_limits references characteristic(s) {unlisted!r} not "
                f"declared in testerkit_characteristics({marker_ids!r}). Add them "
...
@pytest.fixture(autouse=True)
def _testerkit_push_params(request: pytest.FixtureRequest) -> Iterator[None]:
...
@pytest.fixture(autouse=True)
def _testerkit_push_limits(
    request: pytest.FixtureRequest,
    _testerkit_push_params: None,
) -> Iterator[None]:
    """Merge ``testerkit_limits`` markers into typed configs and push them into state.

    Markers are attached to the item during collection from several
    sources, in merge order (later wins): inline
    ``@pytest.mark.testerkit_limits`` decorators → sidecar (file-level →
    ...
    (from ``testerkit_characteristics``, or derived from per-limit
    ...
    for node in request.node.listchain():
        for marker in node.own_markers:
            if marker.name != "testerkit_limits":
                continue
...
@pytest.fixture(autouse=True)
def _testerkit_resolve_connections(
    request: pytest.FixtureRequest,
    _testerkit_push_limits: None,
) -> Iterator[None]:
    ...
    (set by ``_testerkit_push_limits`` from the marker or from per-limit
    ``characteristic:`` values) and ``testerkit_connections`` (explicit
    ...
    than errors.
    """
    from testerkit.execution._state import get_active_test_characteristics

    test_chars = get_active_test_characteristics()
    conn_marker = next(iter(request.node.iter_markers("testerkit_connections")), None)
...
@pytest.fixture(autouse=True)
def _testerkit_apply_mocks(request: pytest.FixtureRequest) -> Iterator[None]:
    """Install mocks declared via ``testerkit_mocks`` markers.
    ...
    for node in request.node.listchain():
        for marker in node.own_markers:
            if marker.name != "testerkit_mocks":
                continue
            try:
...
__all__ = [
    "_testerkit_apply_mocks",
    "_testerkit_push_limits",
    "_testerkit_push_params",
    "_testerkit_resolve_connections",
