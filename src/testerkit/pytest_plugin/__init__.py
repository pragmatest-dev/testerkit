"""pytest plugin for TesterKit test framework."""
...
from testerkit.execution.connections import ConnectionIterator
from testerkit.execution.harness import Context, _ScopedContext
from testerkit.execution.instrument_events import emit_instrument_events
from testerkit.execution.metadata import build_run_metadata
from testerkit.execution.profiles import resolve_test_phase
from testerkit.execution.run_scope import RunContext, RunScope
from testerkit.execution.verify import (
    LimitsFn,
    MeasureFn,
    VerifyFn,
...
from testerkit.fixtures.manager import FixtureManager, PinAccessor
from testerkit.instruments.pool import InstrumentPool
from testerkit.instruments.route_manager import RouteManager
...
from testerkit.models.test_config import FixtureConfig, PromptConfig
from testerkit.parts.context import PartContext
from testerkit.prompts import ask as ask_prompt

# Pytest discovers fixtures by attribute lookup on the plugin module —
# importing them here makes them attributes of ``testerkit.pytest_plugin``
# without exposing them via ``__all__`` (they are framework internals,
# named with leading ``_``).
from testerkit.pytest_plugin.autouse import (
    _reseat_current_run_scope,  # noqa: F401
    _route_cleanup,  # noqa: F401
    _testerkit_apply_mocks,  # noqa: F401
    _testerkit_push_limits,  # noqa: F401
    _testerkit_push_params,  # noqa: F401
    _testerkit_resolve_connections,  # noqa: F401
)
...
from testerkit.pytest_plugin.hooks import (
    VECTORS_MATRIX_KEY,
...
# canonically in ``testerkit.execution._state``.
__all__ = [
    "pytest_addoption",
    "pytest_assertion_pass",
    "pytest_collection_modifyitems",
...
def _build_run_metadata(request: pytest.FixtureRequest) -> dict[str, Any]:
    ...
    from testerkit.execution.profiles import validate_phase_wiring
    from testerkit.store import load_station_type

    requested_phase = request.config.getoption("--test-phase") or os.environ.get(
        "TESTERKIT_TEST_PHASE"
    )
...
def _emit_run_start_events(run_scope: RunScope) -> None:
    ...
    """
    from testerkit.data.events import RunStarted, StepsDiscovered
...
def _run_scope(request) -> Generator[RunScope, None, None]:
    ...
    via ``testerkit show -f X`` and ``testerkit export <run> -f X``.
    ...
    env_uut_serial = os.environ.get("TESTERKIT_UUT_SERIAL")
    if env_uut_serial:
...
def part(request) -> Part | None:
    ...
    """
    from testerkit.pytest_plugin.helpers import is_yaml_path
...
def station_config(request) -> StationConfig | None:
    ...
    """
    from testerkit.execution._state import set_active_station_config
...
def fixture_config(request) -> FixtureConfig | None:
    ...
    by either a worker's ``_TESTERKIT_SITE_INDEX`` env var or a
    ...
    if fc.is_multi_site and fc.sites:
        from testerkit.execution._state import get_current_site_index
...
def instrument_records(request, station_config, mock_instruments) -> dict[str, InstrumentRecord]:
    ...
    from testerkit.instruments.loader import find_instruments_dir, resolve_station_instruments
...
def instruments(
    ...
    When --mock-instruments is passed (or TESTERKIT_MOCK_INSTRUMENTS=1), uses mock
...
def uut(
    ...
    from testerkit.parts.loader import load_part_driver
...
def sync(_run_scope):
    ...
    In worker mode (_TESTERKIT_SITE_INDEX set), returns a SyncPoint that
    ...
    from testerkit.execution.site_runner import is_worker_mode  # checks _TESTERKIT_SITE_INDEX

    if not is_worker_mode():
        yield None
        return

    from testerkit.execution.sync import get_sync
...
from testerkit.execution.vectors import Vector  # noqa: E402
...
def context(_run_scope: RunScope | None) -> Generator[Context, None, None]:
    ...
    resolve session via that var (``testerkit.files.write``,
    ``testerkit.files.stream``, ``testerkit.channels.stream`` indirectly)
...
@pytest.fixture
def connections(
    _testerkit_resolve_connections: None,  # noqa: F811  # pytest fixture-ordering dep
    context: Context,
) -> ConnectionIterator | None:
    """Active fixture connections for the current test.

    Returns the :class:`ConnectionIterator` resolved from
    ``testerkit_characteristics`` / ``testerkit_connections`` markers, or ``None`` when
...
def verify() -> VerifyFn:
    ...
    :func:`testerkit.execution.verify.build_verify_callable` — logs a
...
def measure() -> MeasureFn:
    ...
    Thin pytest wrapper around the runner-neutral
    :func:`testerkit.execution.verify.build_measure_callable`. Both shapes
...
def limits() -> LimitsFn:
    ...
    """
    from testerkit.execution.verify import _LimitsMapping
...
def prompt(request: pytest.FixtureRequest) -> Callable[..., Any]:
    ...
    Resolves prompts declared via ``testerkit_prompts`` markers (file-level,
    class-scoped, per-test, or inline ``@pytest.mark.testerkit_prompts``).
    ...
    itself goes through :func:`testerkit.prompts.ask` — explicit handler
    (UI runner) → ``TESTERKIT_AUTO_CONFIRM=1`` → tty fallback.
    """
    ...
    for node in request.node.listchain():
        for marker in node.own_markers:
            if marker.name != "testerkit_prompts":
                continue
    ...
    def _ask(key: str | None = None) -> Any:
        if key is None:
            if not merged:
                raise pytest.UsageError(
                    f"prompt() called with no key but no testerkit_prompts "
            ...
            if key not in merged:
                ...
                raise pytest.UsageError(
                    f"prompt({key!r}): no such key in testerkit_prompts markers "
