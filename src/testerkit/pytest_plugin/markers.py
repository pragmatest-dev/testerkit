"""Pytest marker translation for TesterKit :class:`TestEntry` fields.

The cascade-merged :class:`TestEntry` carries every TesterKit-marker
field as a typed Pydantic value. This module turns those fields into
``pytest.mark.testerkit_<name>(...)`` calls and attaches them to the
...
* :func:`enforce_no_inline_stacking` — raise a clear error when more
  than one ``testerkit_X`` marker of the same name decorates a test.
* :func:`extract_characteristic_marker_ids` — pull the list of
  characteristic IDs out of a ``testerkit_characteristics`` marker payload.
"""
...
from testerkit.models.test_config import TestEntry

TESTERKIT_MARKER_NAMES: tuple[str, ...] = (
    "testerkit_limits",
    "testerkit_sweeps",
    "testerkit_mocks",
...
"""Names of the TesterKit markers that map 1:1 to ``TestEntry`` fields.
...
class StackedMarkersError(ValueError):
    """Raised when a function carries more than one inline ``testerkit_X`` marker.

    Multi-sweep goes in the single payload list; ``parametrize`` is
    the explicit exception (it's not in :data:`TESTERKIT_MARKER_NAMES`,
    so pytest's native stacking convention stays for it).
    """


def apply_entry_markers(item: pytest.Item, entry: TestEntry) -> None:
    """Attach pytest markers for every non-empty TesterKit field on ``entry``.

    Emits one ``testerkit_<name>`` marker per field, then walks
    ``entry.runner.markers`` for opaque ecosystem markers
    (``flaky`` / ``skip`` / etc.). ``parametrize`` entries are skipped
    here — they're consumed by :mod:`testerkit.pytest_plugin.sweeps`.
    """
    if entry.limits:
        item.add_marker(pytest.mark.testerkit_limits(**dict(entry.limits)))
    if entry.sweeps:
        item.add_marker(pytest.mark.testerkit_sweeps([s.root for s in entry.sweeps]))
    if entry.mocks:
        item.add_marker(pytest.mark.testerkit_mocks(list(entry.mocks)))
    if entry.characteristics:
        item.add_marker(pytest.mark.testerkit_characteristics(list(entry.characteristics)))
    if entry.connections is not None:
        ...
        # marker, matching ``testerkit_characteristics([...])`` shape);
        # dict[str, Any] binds by instrument → channel (passed as
        # kwargs, matching ``testerkit_limits(**by_name)`` shape).
        if isinstance(entry.connections, list):
            item.add_marker(pytest.mark.testerkit_connections(entry.connections))
        else:
            item.add_marker(pytest.mark.testerkit_connections(**entry.connections))
    if entry.retry is not None:
        item.add_marker(pytest.mark.testerkit_retry(**entry.retry.model_dump(exclude_none=True)))
    if entry.prompts:
        item.add_marker(pytest.mark.testerkit_prompts(**dict(entry.prompts)))
...
def normalize_inline_list_payload(
    ...
    Used by inline ``@pytest.mark.testerkit_sweeps`` / ``testerkit_mocks`` /
    ``testerkit_characteristics`` payload extraction. Sidecar / profile cascade
...
def enforce_no_inline_stacking(marker_names: list[str]) -> None:
    """Raise :class:`StackedMarkersError` if any ``testerkit_X`` name appears more than once.
...
def extract_characteristic_marker_ids(payloads: list[Any]) -> list[str]:
    """Extract characteristic IDs from a ``testerkit_characteristics`` marker's payloads.

    ``payloads`` is a list of marker payloads; in practice it always
    has exactly one entry because :func:`enforce_no_inline_stacking`
    rejects multiple ``testerkit_characteristics`` markers on the same
    ...
    if not payload_args:
        raise ValueError("testerkit_characteristics requires at least one characteristic ID.")
    if len(payload_args) == 1 and isinstance(payload_args[0], list):
        ...
        raise ValueError(
            f"testerkit_characteristics payload must be a list of characteristic ID strings "
            f"(or varargs of strings); got {payload_args!r}"
        )
    if not all(isinstance(i, str) for i in ids):
        raise ValueError(f"testerkit_characteristics entries must be strings; got {ids!r}")
    return list(ids)
