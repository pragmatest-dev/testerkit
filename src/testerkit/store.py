"""Centralized persistence for all TesterKit YAML config files.
...
* ``load_project_config`` returns a default ``ProjectConfig(name="testerkit")``
  if ``testerkit.yaml`` is missing instead of raising; use ``load_project``
...
from testerkit.utils.paths import (
    _resolve_root,
    get_fixture_paths,
...
def detect_file_type(path: Path) -> str | None:
    """Auto-detect the TesterKit file type from YAML structure.

    Real TesterKit YAML files never carry a wrapper key literally named
    ...
    *names*, not YAML keys the models define. Detection instead keys off
    ...
    - ``catalog`` (:class:`~testerkit.models.catalog.InstrumentCatalogEntry`):
      the only model with a top-level ``capabilities`` list.
    - ``part`` (:class:`~testerkit.models.part.Part`): ``characteristics``
    ...
    - ``instrument_asset`` (:class:`~testerkit.models.instrument_asset.InstrumentAssetFile`):
      ``protocol``/``driver`` (connection info) or ``info``/``calibration``
      (per-unit identity) at the top level.
    - ``station`` (:class:`~testerkit.models.station.StationConfig`):
      ``instruments`` dict — no other model defines this key.
    - ``fixture`` (:class:`~testerkit.models.test_config.FixtureConfig`):
      ``connections`` (single-UUT) or ``sites`` (multi-UUT) — mutually
      exclusive with each other and unique to fixtures.
    - ``project`` (:class:`~testerkit.models.project.ProjectConfig`,
      i.e. ``testerkit.yaml``): the only model with no ``id`` field at all,
...
def _write_model(path: Path, model_data: dict[str, Any]) -> None:
    """Write a model dict to YAML using TesterKit formatting conventions."""
...
def load_project(path: Path) -> ProjectConfig:
    """Load and validate a testerkit.yaml project config file.
    ...
    conflict between an inline ``testerkit.yaml: profiles:`` entry and a
...
def load_project_config(project_root: Path | None = None) -> ProjectConfig:
    """Load ``testerkit.yaml`` from a project root.

    Args:
        project_root: Project root directory. If None, defaults to cwd.

    Returns:
        Validated ProjectConfig model, or a default ``ProjectConfig(name="testerkit")``
        if ``testerkit.yaml`` is not present in the resolved root.
    """
    path = _resolve_root(project_root) / "testerkit.yaml"
    if not path.exists():
...
def find_station_config(
    ...
    *,
    ...
    2. ``~/.local/share/testerkit/stations/{station_id}.yaml`` (machine-global)
...
def find_catalog_dirs(*, project_root: Path | None = None) -> list[Path]:
    ...
    Server should be run from the project root (e.g., `cd demo && testerkit serve`),
    so `catalog/` resolves to that project's catalog.

    Also includes bundled generic catalog from the testerkit package.
    """
    dirs: list[Path] = []
    root = _resolve_root(project_root)
...
def format_file(path: Path) -> str:
    """Load a YAML file and return it formatted with TesterKit conventions."""
    return dump_yaml(_read_yaml(path))


def format_file_inplace(path: Path) -> bool:
    """Format a YAML file in-place with TesterKit conventions.
...
# =============================================================================
#
# Any list-producing position in a TesterKit YAML file accepts a single-key
...
def _fmt_apply_style(data: Any, key: str | None = None) -> Any:
    """Recursively apply TesterKit YAML style rules."""
    if isinstance(data, dict):
        cm = CommentedMap()
        ...
        # testerkit.yaml (exclude_defaults) can legitimately be all-scalar
...
def dump_yaml(data: dict[str, Any]) -> str:
    """Dump a dict to a YAML string with TesterKit conventions."""
