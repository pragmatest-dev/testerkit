...
description: Use when a user wants a computed statistic across Litmus runs — yield (FPY/RTY/DPMO), a Pareto of top failures, Ppk/Pp capability, a yield trend, a retest rate, or time-loss. Reading or exporting the underlying records is litmus-data; why one run failed is litmus-debug.
---
...
detail, or an export is `litmus-data`; diagnosing a single failure is
`litmus-debug`.
...
| Question | Reach for |
|---|---|
| First-pass / final yield, RTY, DPMO, DPPM, counts | `litmus metrics summary` |
| Top-N failing parts / steps / measurements | `litmus metrics pareto --group-by part\|step\|measurement` |
| Process capability (Ppk / Pp) | `litmus metrics ppk --min-samples N` |
| Yield bucketed over time | `litmus metrics trend --period day\|week\|month` |
| Retested-serial count, retest rate, avg retries | `litmus metrics retest` |
| Total/pass/fail/error seconds per period | `litmus metrics time-loss` |
...
- **Metrics already exclude `development`-phase noise** and stay stable across
  schema changes — reach for `litmus metrics` or the Query API, never a
  hand-rolled aggregate over the data files.
- **`litmus serve` → `/metrics`** for a human — the same `MeasurementsQuery`
...
```
Sibling skills: `litmus-data` (read/export the records these metrics compute
over), `litmus-debug` (why one run failed), `litmus-capture` (channels/files
