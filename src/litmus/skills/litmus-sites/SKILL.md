...
single UUT at a time; see `litmus-tests` instead.
...
physical box — Litmus serializes calls to it so two sites never collide
mid-measurement, with no lock code in the test. Under
`--mock-instruments` (see `litmus-mocks`) each site additionally gets
...
`site_index` and `site_name` are columns on every run row. `litmus runs
--json` and `litmus show <run_id>` surface them per run; `RunsQuery`'s
...
litmus validate                  # fixtures/*.yaml against FixtureConfig
```
...
Read the docs:
```bash
litmus docs show how-to/execution/multi-uut-testing
```
