...
walkthrough) live in the shipped `litmus docs show
...
on. Litmus's live data — `ChannelStore` samples, `EventStore` events — is
produced on background gRPC Flight reader threads. Every Litmus-provided
...
|---|---|---|---|
| `litmus.ui.channel_data(id).subscribe(cb)` | `litmus.ui` (re-export of `litmus.ui.shared.event_binding.ui_channel_data`) | NiceGUI event loop (`_on_ui_loop` hops it there) | **Yes** — one element, one sample, one line |
| `litmus.ui.subscribe(store, cb, ...)` (`ui_subscribe`) | same module, for `EventStore` events (session/instrument activity) | NiceGUI event loop | **Yes**, same reason |
| `litmus.channels.latest(name, cb)` | `litmus.channels` (store-direct, no NiceGUI dependency) | raw Flight reader thread | **No** |
| `litmus.channels.live(name, cb, max_hz=...)` | `litmus.channels` | raw Flight reader thread | **No** |
| `litmus.channels.window(name, cb, dur=...)` | `litmus.channels` | raw Flight reader thread (history prefill runs inline, then the same raw thread for the live tail) | **No** |
...
`litmus.channels` verbs are for scripts, notebooks, and anything with no
...
Read `src/litmus/ui/components/channel_values.py` before writing a new
...
`litmus/ui/__init__.py`'s own module docstring (not this skill) shows:
...
`litmus/ui/shared/event_binding.py`: `_channel_signals: dict[str, Event]`,
...
`"latest"`-style binding surface in Litmus today — every live view is
...
- **`utc_datetime_input`** exists in the shared UI components but has no
  ...
  code.
- **`InstrumentToggle`** (`litmus.ui.shared.components`) is the shipped
