"""Custom-UI-builder surface for Litmus.
...
store so your page updates push-style as new samples arrive::
    ...
    from litmus.ui import subscribe, channel_data, bind_channel_store
...
The deep ``litmus.ui.shared.components`` and
``litmus.ui.shared.event_binding`` paths still work — they're the
contributor form. User code should use this module.

Operator pages themselves (``/results``, ``/metrics``, etc.) are
registered via ``litmus.ui.pages.*`` on package import and don't
