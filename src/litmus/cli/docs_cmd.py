...
Single-source replacement for the removed ``litmus refs`` command: instead of
...
and ``litmus.ui.pages.docs.page._resolve_docs_dir``, which this mirrors).
"""
...
# Sections that ship as user-facing docs (mirrors litmus.ui.pages.docs.page).
...
def _docs_dir() -> Path:
    ...
    Wheel installs bundle the curated user-facing tiers at ``litmus/_docs/``.
    Editable / source installs (this dev repo) don't get the bundle, so fall
    back to the repo's ``docs/`` directory above ``src/litmus/``.
    """
...
def docs_show(path: str):
    ...
    Example:
        ...
        litmus docs show how-to/execution/operator-prompts
    """
    root = _docs_dir()
