...
class TestMockDMM:
    ...
    def test_default_values(self):
        dmm = Mock(DMM)
        assert dmm.measure_voltage() == 0.0
        assert dmm.measure_current() == 0.0
        assert dmm.measure_resistance() == 0.0
        assert dmm.measure_frequency() == 0.0

    def test_custom_values(self):
        dmm = Mock(DMM, measure_voltage=3.3, measure_current=0.5, measure_resistance=470)
        assert float(dmm.measure_voltage()) == 3.3
        assert float(dmm.measure_current()) == 0.5
        assert float(dmm.measure_resistance()) == 470.0
    ...
    def test_set_mock_value(self):
        ...
        assert float(dmm.measure_voltage()) == 5.0
    ...
    def test_period_from_frequency(self):
        ...
        assert float(dmm.measure_frequency()) == 1000.0
        assert float(dmm.measure_period()) == 0.001
    ...
    def test_raw_scpi_responses(self):
        ...
        assert float(dmm.measure_voltage()) == 3.3
        assert float(dmm.measure_current()) == 0.1
...
class TestMockPSU:
    ...
    def test_enable_output(self):
        ...
        assert psu.measure_output_voltage() == 5.0
        assert psu.measure_output_current() == 1.0
    ...
    def test_context_manager(self):
        with Mock(PSU, measure_voltage=12.0) as psu:
            psu.enable_output()
            assert psu.measure_output_voltage() == 12.0
    ...
    def test_set_mock_value(self):
        ...
        assert psu.measure_output_current() == 0.25
    ...
    def test_convenience_methods(self):
        ...
        assert psu.measure_current() == 0.5
...
class TestMockELoad:
    ...
    def test_constant_current_mode(self):
        ...
        assert eload.measure_power() == 5.0
    ...
    def test_constant_resistance_mode(self):
        ...
        eload.set_load_resistance(100.0)
        eload.enable_load()

        assert eload.mock_state["mode"] == "CR"
        assert eload.measure_power() == 1.0
    ...
    def test_set_mock_value(self):
        ...
        assert float(eload.measure_voltage()) == 3.3

        eload.set_mock_value("measure_power", 10.0)
        assert float(eload.measure_power()) == 10.0
