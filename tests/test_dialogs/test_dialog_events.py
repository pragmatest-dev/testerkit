...
from testerkit.api.dialogs.manager import DialogManager
from testerkit.api.dialogs.models import ConfirmDialog, DialogResponse
...
async def test_auto_respond_emits_both_events() -> None:
    ...
    with patch("testerkit.api.dialogs.manager.get_current_run_scope", return_value=mock_logger):
...
async def test_no_logger_no_events() -> None:
    ...
    with patch("testerkit.execution._state.get_current_run_scope", return_value=None):
...
async def test_local_dialog_emits_events() -> None:
    ...
    old_auto = os.environ.pop("TESTERKIT_AUTO_CONFIRM", None)
    try:
        with patch("testerkit.api.dialogs.manager.get_current_run_scope", return_value=mock_logger):
        ...
        if old_auto is not None:
            os.environ["TESTERKIT_AUTO_CONFIRM"] = old_auto
