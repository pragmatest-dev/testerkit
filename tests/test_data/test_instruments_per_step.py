"""Verify that step/vector rows carry per-step instrument subsets (feature B).

The run row keeps the full connected inventory; each step row carries
only the instruments the step requested via its fixture parameters.
"""
...
class TestInstrumentsPerStep:
    def test_step_with_dmm_only_excludes_psu(self, tmp_path):
        ...
        assert step_names == ["dmm"], "step row must carry only the dmm"

        vector_names = sorted(r["name"] for r in rows["vector"]["instruments"])
        assert vector_names == ["dmm"], "vector row must carry only the dmm"
    ...
    def test_step_with_no_instruments_produces_empty_set(self, tmp_path):
        ...
        assert rows["step"]["instruments"] == [], "step with no instruments must be empty"
        assert rows["vector"]["instruments"] == [], "vector with no instruments must be empty"
