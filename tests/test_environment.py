"""Tests for testerkit.environment — environment snapshot capture."""

from testerkit.environment import (
    EnvironmentSnapshot,
    capture_environment,
)
...
class TestEnvironmentSnapshot:
    def test_roundtrip_json(self):
        snap = EnvironmentSnapshot(
            ...
            dependencies=["testerkit>=0.1.0", "pytest>=8.0"],
        ...
        assert restored.dependencies == ["testerkit>=0.1.0", "pytest>=8.0"]
...
class TestCaptureEnvironment:
    def test_returns_snapshot(self):
        ...
        assert snap.os_name
        assert snap.testerkit_version

    def test_dependencies_from_pyproject(self):
        snap = capture_environment()
        # We're running in the testerkit repo, so pyproject.toml exists
