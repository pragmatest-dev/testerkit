"""``TesterKitClient`` results-API outcome propagation.

The catch-all results API (``TesterKitClient`` / ``RunBuilder``) must stamp
...
def test_passing_measure_step_propagates_passed(tmp_path):
    client = TesterKitClient(data_dir=tmp_path)
