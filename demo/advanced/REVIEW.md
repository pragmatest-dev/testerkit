...
Notes gathered while building `demo/advanced/` (PMIC-A23, 14 tests across
...
`demo/advanced/conftest.py` only exists to prepend `demo/..` to
`sys.path` so `from demo.drivers import ...` works. Any user who
...
- **#6** — `SpecBand.when` accepts bare scalars (`{load: 0.1}`) via
  the existing Pydantic union type. `demo/advanced/products/pmic_a23.yaml`
...
- **PC3** — `demo/bringup/` created as the Tier 0/1 reference. Three
  tests, one conftest, one sidecar, no station / product / fixture
  YAML. `demo/pytest_native/` and `demo/advanced/` remain the higher
  tiers.
...
`demo/` should model the arc directly:

- `demo/bringup/` — one test file, one conftest, no YAML. Shows
  Tier 0/1. Opens in a fresh IDE and just runs.
- `demo/pytest_native/` — add station + product + sidecar. Shows
  Tier 2.
- `demo/advanced/` (exists) — full production flow with profiles,
  catalog, multi-pin bindings. Shows Tier 3/4.

A user reads them in order and sees the platform progressively
reveal itself. Today `pytest_native` is the only demo and jumps
straight to full-featured — the bringup path is invisible.
