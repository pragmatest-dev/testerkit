"""Multi-DUT smoke test — parallel power-up with a cross-DUT sync barrier.

Each DUT runs in its own subprocess (``LITMUS_SLOT_ID`` set) via the
main Litmus plugin's slot runner; the ``sync`` fixture blocks until
every slot reaches a shared barrier. Tests themselves are slot-unaware
— same class, same methods, same fixtures.
