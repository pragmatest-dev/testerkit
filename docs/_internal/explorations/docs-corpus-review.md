...
README.md, CHANGELOG.md, ROADMAP.md, the testerkit-starter repo (README/WELCOME), and the
pragmatest.com landing copy (`src/app/testerkit/page.tsx`). pragmatest syncs `docs/` via
`scripts/sync-testerkit-docs.mjs`, so fixing `docs/` propagates — never double-edit synced
content.
...
- **`pip install testerkit` is the headline, universal install.** Works without uv. Use
  it as the primary install command everywhere.
- **uv is the testerkit repo's own dev tool** (and a fine power-user choice for user projects),
  but is **not required for users**. Don't imply it is.
- The `testerkit init` scaffold produces a standard PEP 621 `pyproject.toml` (`init.py:127`) —
...
- Don't mix `pip install` and `uv sync` within one flow without noting they're two tools
  (the quickstart bug: `pip install testerkit` only fetches the CLI; `uv sync` then builds
  the project venv).
- **Examples stay on the uv workspace.** uv hard-errors if a workspace member drops
  `testerkit = { workspace = true }` (verified 2026-06-23) — the line is mandatory for
  local-HEAD testing. Examples are repo-internal and not part of the user install story.

Known out-of-scope code follow-up (flagged, NOT changed in this sweep): `testerkit init` prints
...
- **Non-pytest / existing suites integrate via two real, shipped surfaces:** the imperative
  ...
  loops") and the `TesterKitClient` results API (`src/testerkit/client.py:345`,
...
### testerkit_match surface (2026-06-23, verified)

`testerkit_match(requirements=[...])` is an **MCP tool only** (`src/testerkit/mcp/server.py:363`).
NOT a Python function, NOT in any `__all__`, NO CLI `match` command, and HTTP `GET /match`
accepts only `part_id`/`station_id` (`src/testerkit/api/app.py:737`) — not the ad-hoc
...
3. `testerkit.queries` field-ref helpers (`ColumnSchema`/`FieldRef`/`FieldRole`) — exported from
   the public Query API namespace, only incidentally mentioned; no defining entry.
4. `TESTERKIT_CHANNELS_SYNC_PUSH` — channels tuning knob missing from cli.md Environment Variables.
...
- **FOUR-store model (NOT three).** There are 4 user-facing stores: EventStore, ChannelStore,
  ...
  **RESOLVED 2026-06-24: user chose FULL sweep + file rename.** `three-stores.md` → `data-stores.md`
  ...
  ontology testerkit.yaml docs: paths fixed. NOTE pre-existing dangling ref: ontology L721
...
- **Source nits (commit 60e7fa40):** parquet.py save-docstring filename comment → real `{timestamp}_{run_id8}_{serial}`
  ...
  added a regression test (test_catalog/test_loader.py ...appends_signal_bands); **TesterKitClient default `data_dir`
...
- **Operator-UI content audit — ALL 19 pages DONE** (deferred by the 06-24 method revision; 2-lens per page, verified vs
  `src/testerkit/ui/pages/*`). Commits a125ed6c / fbbb30aa / 8c87c603 / e3a4b84e / c3fd29c0 / 4102ba4b. Real bugs: metrics
...
the testerkit docs render via the SEPARATE pragmatest Next.js site, synced from docs/), so a live pass would mean standing up +
...
- **Stale-fact sweep (whole corpus):** "twelve MCP"=0, "uv pip install testerkit"=0, "is a pytest plugin"=0. Fixed the last 3
...
human review); (c) the tracked source nits (parquet.py:224-225, connections.py:411, store.py:946-955, TesterKitClient ./results
default); (d) testerkit.channels declare/write_many doc home; (e) merge the 2 external topic branches (testerkit-starter, pragmatest).

### Piece 7 — external surfaces (separate repos; changes pushed as TOPIC BRANCHES, NOT to main — user merges)
- testerkit-starter (repo `/home/ryanf/repos/testerkit-starter`, branch `docs/corpus-review-fixes`, pushed): README.md +
  .devcontainer/welcome.txt reviewed clean (pip-forward, real commands, `testerkit data prune` real, `testerkit init --starter`
...
- pragmatest.com (repo `/home/ryanf/repos/pragmatest.com`, branch `testerkit/corpus-review-part-vocab`, pushed): landing
  `src/app/testerkit/page.tsx`. Mostly sound — platform framing correct ("Your platform for hardware test"; pytest-native not
  "is a plugin"), `pip install testerkit`, MCP+CLI+skills, Parquet/Arrow formats, codespaces + quickstart links all accurate.
  Fixed `products`→`parts` in 2 config-entity lists + 1 capability-matching line (testerkit's DUT entity is `part`/`parts/`, per
  the product→part rule). LEFT the general hero line "Mapping product specs to a test plan" (top-of-funnel plain English).
  The page's `getProduct('testerkit')` infra correctly uses "product"=business-product (legit). Branch pushed; needs user merge.
  ⚠️ FLAG FOR USER: the product→part edit on marketing copy is a judgment call — revert the one-commit branch if "product"
  was intentional for top-of-funnel accessibility. pragmatest's testerkit DOCS are synced from docs/ (already fixed Pieces 1-6);
...
- Coverage gaps re-verified (scan was 06-23): gap3 (testerkit.queries FieldRef/ColumnSchema/FieldRole) CLOSED — now defined in
  models.md (L1526/1555/1741). gap5 (XYData) PRESENT — fixed its source docstring "(item 15)" internal-roadmap leak +
  regenerated models.md. gap4 (env vars) DONE — added TESTERKIT_FILES_BACKEND + TESTERKIT_CHANNELS_SYNC_PUSH to cli.md (the table is
  curated to user-facing knobs; the other 12 missing TESTERKIT_* are internal orchestrator-injection/debug vars, correctly excluded).
...
- reference/configuration — (effectively a full hand-written schema reference; only the L9-19 index is generated). Accuracy:
  ...
  for the hand-written ones. Audience: dropped `_INSTRUMENT_TYPE_ALIASES`/`testerkit.store`, `validate_station_against_type(...)`
  signature, `patch.object` (×2), the sidecar-loader "because it lives in testerkit.execution.sidecar" rationale. ✅
- reference/cli — accuracy CRIT: "Auto-detect — `production` if git clean" is WRONG; `resolve_test_phase(None)` returns
  ...
  (--test-phase / TESTERKIT_TEST_PHASE / default development) + the git/mock enforcement table + row. Profile YAML does NOT feed the
...
- ⚠️ OPEN GAP for USER (gaps 1-2, NOT auto-fixed — needs a structural decision): `testerkit.channels` `declare` + `write_many` are
  ...
  page for the full testerkit.channels API, or fold declare/write_many into stream-live-channel.md. Did not auto-create a page.
...
- reference/index — accuracy: `twelve MCP tools`→dropped the count (real count is 13; api.md is the
  ...
  (`@pytest.mark.testerkit_*`) is CORRECT + stable (frozen `TESTERKIT_MARKER_NAMES` 7-tuple) → left. ✅
...
- reference/pytest/markers — accuracy 1 CRIT (link `configuration.md#sidecar-yaml`→`../configuration.md#sidecar-yaml`;
  ...
  Audience CRIT: L3 dropped the `TESTERKIT_MARKER_NAMES` constant + `src/...markers.py` path. KEPT `StackedMarkersError`
  (user-visible collection-error name, same class as the retained UsageError/MissingLimitError). 30 claims verified
  correct. STRUCTURAL ADD (tracked Piece-5 task from writing-tests.md): added the `testerkit_characteristics` ×
  `testerkit_connections` composition matrix (11 cases + invariants) — VERIFIED line-by-line against connections.py +
...
- reference/pytest/fixtures — accuracy 0 CRIT, 2 WARN: `context.limits` was equated "(= `limits` fixture)" but they
  ...
  path + private fixture names (`_route_manager`/`_testerkit_push_params`); L332 `hooks.py:232–274` file:line → all removed.
  2 WARN: "currently-bound connection"→"connection currently being iterated"; `_TESTERKIT_SLOT_ID`→"(multi-slot)".
...
- reference/runtime/connect — accuracy: 1 WARN only (`connect()` sig + table said `data_dir: Path|str|None`; source is
  ...
  per-resource locking, station-config resolution). Audience CRIT L5 `Source: src/testerkit/connect.py` removed; scrubs:
...
- reference/runtime/client — accuracy: 28 claims verified, 0 issues. Two of MY suspicions were WRONG: `run.id` IS real
  ...
  IS the real TesterKitClient default. Audience CRIT L148: internal `TestVector`/`TestStep` model names → builder vocab. DECLINED
  the audience agent's "add 'same store as every runner' framing" — accuracy shows TesterKitClient defaults to `./results`, NOT
  the pytest/connect() testerkit.yaml data_dir, so that framing would be FALSE by default. Heading-case/anchors left intact
  (#from-labview/#querying-results are link targets). **runtime reference sub-cluster (3 pages) DONE.** ✅
  ⚠️ TRACKED DIVERGENCE (code design, not a doc bug): `TesterKitClient(data_dir="results")` hardcoded default ≠ the
  `resolve_data_dir()`/testerkit.yaml convention pytest + connect() use. So TesterKitClient writes to ./results by default while
  ...
  (Piece 4b, already closed) says "the same store every TesterKit runner writes to" — mildly overclaims given this default.
...
- reference/catalog/schema — accuracy: 2 CRIT broken links (`catalog-cookbook.md`→`cookbook.md` at L5 + L372) + 1 WARN
  fragile anchor (`models.md#catalog-entry--testerkitmodelscatalog`→ stable `#model-instrumentcatalogentry`, matching
...
- reference/data/performance-limits — accuracy: 38 claims verified, 0 CRIT/WARN; numbers correctly framed as empirical (not
  ...
  ceilings/cliffs + `testerkit benchmark` repro path. **data reference sub-cluster (4 pages) DONE.** ✅
...
examples; pytest-existing + harness shared the testerkit.execution.logger→run_scope stale import; pytest-existing
stale "PyPI release coming" (testerkit IS live 0.2.0); instruments had 3 fabricated YAML shapes (raw-PyVISA
...
- integration/runtime/harness — CRIT accuracy: the SAME `testerkit.execution.logger`→`run_scope` import fix
  (stale logger rename; this page is the right home for the TestHarness/RunScope detail so I fixed the
  import rather than removing it). Audience (3 structural CRITs): added the TesterKitClient-vs-TestHarness
  ...
  path (point first-timers at TesterKitClient); added a "confirm the run landed" step (testerkit runs / testerkit show).
...
- integration/runtime/pytest-existing — CRIT accuracy: `from testerkit.execution.logger import RunScope` is a
  ModuleNotFoundError (the module was renamed logger→run_scope) — resolved by demoting Path C to a harness.md
  pointer (wrong audience for inline TestHarness/RunScope anyway). Install: git-clone + `uv add ~/src/testerkit`
  + "PyPI release coming" → `pip install testerkit` (verified testerkit IS live on PyPI 0.2.0 — the stale
...
- integration/data/logging — 2 CRIT (non-runnable examples) + 1 CRIT accuracy: the flagship logging.Handler
  ...
  to lead with testerkit runs/show/HTTP (raw glob = escape hatch). docs-writer; re-audit 0/0. ✅
...
- integration/data/results-api — factually accurate (12 claims; correctly documents the non-obvious truth
  ...
  goes via the Python TesterKitClient). Audience: dropped the "see the open follow-up" TOMBSTONE (self-contained
  caveat); "the pytest plugin writes to"→"every TesterKit runner writes to" (platform framing); softened "every
  method signature"; ADDED a verified TesterKitClient submit snippet (start_run→step.measure→run.finish, mirrors
  client.md). ✅
- integration/data/grafana — 3 CRIT accuracy (the page predated today's measurement_values source fix):
  ...
  the brittle "ten dashboards" counts (heading/diagram/intro), removed the `src/testerkit/grafana/dashboards/`
  source-path leak (→ `testerkit grafana export`), uv-pip→pip, "operators pick part"→DUT part number,
...
- how-to/overview/mcp-integration — accuracy: testerkit_discover scans ALL protocols (not VISA-only);
...
- how-to/catalog/datasheet-to-test — CRIT accuracy: sidecar `vectors:`→`sweeps:` (SidecarConfig extra=forbid
  ...
  model"→"catalog schema", derivation narration→action+link, "highest-leverage flow TesterKit ships" hype→
  factual, "six phases"(1/2/2b/3/4/5)→"the phases". pip install testerkit LEFT (correct user install).
...
- how-to/configuration/mock-mode — factually PERFECT (27 claims: Mock(object,…) substitution [not driver
  ...
  CLI>env>testerkit.yaml, testerkit_mocks cascade file→class→test→profile, test_phase auto-demote, deleted
...
- how-to/data/index — accuracy: removed ATML from the export interchange list (`testerkit export` =
...
- how-to/data/benchmarking — in good shape (18 claims: `testerkit benchmark` CLI, --full/--rounds/-o/--no-save
  ...
  coefficients"→"per-operation time and size". (pip install 'testerkit[benchmark]' correctly LEFT — pip
  is the user workflow; audience agent's uv suggestion declined.) ✅
- how-to/data/mcp-query-runs — factually PERFECT (34 claims: testerkit_runs/steps/metrics + 6 metrics actions
...
- how-to/data/mcp-debug-failures — factually PERFECT (27 claims: testerkit_runs/steps/events/sessions/
...
- how-to/data/grafana-dashboards — accuracy: the `measurements` SQL table is RAW NESTED run rows (the view
  ...
  is in the view defs). Audience: `pip install 'testerkit[grafana]'` (quoted; pip is the USER workflow —
...
- how-to/data/find-flaky-tests — tone correctly frames flakiness as investigate-the-hardware (no
  ...
  fixture only — wrong for the page's own unswept example); reframed `testerkit_retry` from "can't fix root
...
- how-to/data/export-results — factually PERFECT (24 claims: `testerkit show -f html/pdf/json/csv` +
  `testerkit export -f csv/json/stdf/hdf5/tdms/mdf4` BOTH real; -t templates HTML/PDF only; exports/<fmt>/
  ...
  leaks; added the PDF extra `pip install 'testerkit[pdf]'`. docs-writer. ✅
- how-to/data/compare-runs — factually PERFECT (18 claims: SQL UNNEST(measurements), record_type=vector,
  struct fields name/value/outcome/limit_low/high, /results+/channels routes, testerkit show -f csv). Audience:
...
- how-to/data/stream-live-channel — factually PERFECT (18 claims verified: channels.stream/.write,
  ...
  `stream` fixture vs store-direct `testerkit.channels.stream` distinction (key gap); de-jargoned
  "subscriptions", trimmed store-on-disk narration. docs-writer. ✅
- how-to/data/capture-an-artifact — CRIT accuracy: removed the `load_file` from `testerkit.data.backends.parquet`
...
- how-to/data/capture-waveform — in good shape (13 claims verified: observe→ChannelStore routing,
  ...
  Waveform import `testerkit.data.models`→`testerkit` (match the examples). Audience: observe URI-stamp narration
...
- how-to/execution/managing-sessions — 2 CRIT accuracy: the whole Data Retention section was fabricated
  (a `testerkit.yaml` `retention:` key — ProjectConfig is extra=forbid, would ValidationError; a nonexistent
  `~/.config/testerkit/config.yaml` global file) + labeled the REAL `testerkit data prune` command "(planned)".
...
- how-to/execution/operator-prompts — factually accurate (24 claims: testerkit_prompts confirm/choice/input
  types, prompt fixture, PromptUnavailableError, TESTERKIT_AUTO_CONFIRM all verified). Audience: retitled
  ...
  dropped the `src/testerkit/api/dialogs/` source link; blocks→waits, dialog-manager→prompt, abstraction heading. ✅
- how-to/execution/multi-uut-testing — accuracy: parquet filename +run_id8; --fixture multi only with 2+
  ...
  mocks not shared" + sync.wait behavior; dropped private `_TESTERKIT_*` env rows. docs-writer; re-audit 0/0. ✅
- how-to/execution/vector-expansion — factually accurate (18 claims: testerkit_sweeps shape, linspace/
  arange/logspace/geomspace/repeat/range generators, vectors fixture all verified). Fixed error-text
  `testerkit_sweeps zip`→`sweep zip`. CRIT audience: cut competitor design-validation (TestStand/OpenTAP/
...
- how-to/execution/profiles — 2 CRIT audience (no create-task block → added "Create and run a profile";
  ...
  file-metadata not a column, `testerkit show` does NOT display profile name/description (removed false
...
- how-to/execution/spec-driven-testing — factually PERFECT (38 claims verified: characteristic= kwarg,
  ...
  (`Part.pins[primary_pin_id].name`, "ContextVars from the driver layer", `testerkit.execution.verify`),
...
- how-to/execution/limits — CRIT accuracy: removed `expr`/`lookup`/`steps`/`callable` from the
  ...
  precedence rule; cut resolver/short-circuit/vector-params jargon + the `testerkit.execution.verify`
  import path. Cascade direction (inline<sidecar<profile, last-wins) verified. ✅
- how-to/execution/writing-tests — CRIT: 11-row testerkit_characteristics×testerkit_connections resolution
  ...
  resolved_pins/testerkit.execution.verify internals; lifecycle/first-class-container jargon. Verified touched
...
- concepts/overview/ai-integration — accurate (13 claims; page makes few specific claims, all
  correct — no MCP-count to go stale, testerkit_run mock-only not violated). schema→config/YAML jargon,
  added MCP one-line gloss, dropped `src/testerkit/skills/` path leak. ✅
- concepts/overview/pytest — accurate (platform-not-plugin framing correct; testerkit_retry/flaky verified).
  ...
  DROP (8 pages, no number anywhere) + fixed 12+ broken `testerkit-fixtures.md`/`testerkit-markers.md` →
...
- concepts/overview/architecture — four-store table VERIFIED correct (sweep held; RunStore not
  ParquetBackend; FileStore present). Fixed Framework→TesterKit heading (platform conflation), lead diagram
...
- ONTOLOGY docs-ref check (src/testerkit/ontology/testerkit.yaml) — 10 stale FLAT doc paths fixed to their real
...
- concepts/overview/platform-vs-framework — CRIT MCP count 12→13 (+`testerkit_files`, `Cpk`→`Ppk`);
...
- Piece 1 tutorial/index.md — 2026-06-23 — fixed 3 criticals (Batteries-included opener
  removed; canonical `pip install testerkit` added to Quick Start block + Prerequisites,
...
- Piece 1 how-to/overview/mcp-integration.md — 2026-06-23 — DEEP drift, 4 audit passes.
  Fixed 10 initial criticals (tool count 12→13 + `testerkit_files` row; 5 uncallable query
  ...
  deeper pre-existing (`testerkit_run` ALWAYS `--mock-instruments`, `tools.py:1128`). Final
  ...
  CODE BUG to flag: `testerkit setup show` (`setup_cmd.py`) prints a STALE hardcoded tool list
...
- Piece 1 reference/overview/skills.md — 2026-06-23 — fixed 10 criticals (tool count 12→13 +
  `testerkit_files`; prereq + stdio-spawn model added; ASCII three-layer diagram → markdown list
  killing the "Task tool" jargon; "model tier→source file" softened; "confabulate"→neutral;
  `prompts/get` protocol jargon → plain; wrong `src/testerkit/skills/` install path → `testerkit/skills/`).
...
- Piece 1 CHANGELOG.md — 2026-06-23 — fixed 6 criticals: 4 accuracy (all describing APIs
  ...
  `@testerkit`→pytest-native (0.1.0 never shipped it) and scrubbed the `test_perf_daemon.py`
...
- 05-configuration — 2 CRIT: (a) `get_param("key")` does NOT raise, returns None/default
  ...
  WINS (cascade appended after inline). Plus `@pytest.mark.flaky`→`testerkit_retry` (respects the
  no-flaky axiom; testerkit_retry wraps rerunfailures) + `changed()` first-vector + jargon. Re-audit 0. ✅
...
- 07-real-instruments — `Zero`→`None` mock default; `measure`→`verify` (limit was inert with measure);
  `testerkit_mocks`(marker) vs `mocks:`(sidecar key) disambiguated; `--station=bench_1` id form; jargon. ✅
...
- Piece 1 ROADMAP.md — 2026-06-23 — fixed 1 tone critical (L1277 "limit-setting today is
  ...
  "`testerkit.pytest_plugin` → `testerkit.pytest_plugin`" (typo; intended target unknown).
...
- Piece 2 tutorial/01-first-test.md — 2026-06-23 — full rewrite resolving 6 structural
  ...
  repo-dev clone+uv-sync install). Now centers on the real `testerkit init --tier=bringup`
  ...
  `testerkit.yaml` has NO `data_dir` — `data_dir: data` is bench/starter-only (`init.py:73-74,234`),
