...
- 2026-07-03 — **Phase 6 (vectors split) landed.** Forks resolved (logged, not stopped — mechanical/parity choices, none change the end-state table map):
  1. **Steps PK includes `vector_outer_index`** (contract's stated steps PK `(run_id, step_path, step_retry)` omits it). A method step run under two different outer class-sweep iterations is TWO distinct executions (own inputs / outcome / timing); collapsing them on a 3-col PK would fuse them. The at-rest grain law (`_row_helpers.RunParquetRow`) already keys steps `(run_id, step_path, step_retry, vector_outer_index)`. So `steps_materialized` PK = `(run_id, step_path, step_retry, vector_outer_index_key)` and `vectors_materialized` PK = that + `(vector_index, vector_retry)`. Consistent (vectors PK = steps PK prefix + inner coords), matches the contract's vectors PK exactly.
  2. **`inputs_map`/`outputs_map` restored to the `steps`/`step_vectors` VIEWS for LIVE runs.** The first pass moved step inputs/outputs to a query-time join against the `inputs`/`outputs` TABLES (empty for a not-yet-ingested run) — a silent live-run regression (no test covered it). Fix: the views now carry `inputs_map`/`outputs_map` — constant-empty on the materialized side (finalized values still come from the tables via `StepsQuery._STEP_IO_JOINS`, keeping the shared views lean so metrics reads aren't taxed), passed through from the inflight overlay on the live side. `StepsQuery` COALESCEs the finalized join over the view's live column. Both live and finalized now render inputs/outputs.
  3. **`vector_retry` added to the inflight overlay** (`INFLIGHT_STEPS_SCHEMA` + the accumulator's vector snapshot). `vectors_materialized` keys on `vector_retry` (contract), so a live retried vector needs its own retry on the overlay to match its finalized row 1:1 — closes a latent gap (the overlay previously carried only the enclosing `step_retry` on vector rows). `step_vectors` still does NOT expose `vector_retry` to `StepRow` (out of scope; StepRow unchanged).
  4. **`step_index` NOT stored on `vectors_materialized`** (strict no-ancestor-data) — the `step_vectors` view joins `steps_materialized` for the enclosing step's `step_index`/`step_name`.
- 2026-07-03 — **Phases 7 + 8 (measurements full-normalize + ordinal/index) landed.** Forks/decisions (logged):
  1. **`step_index` kept as a COORDINATE on `measurements_materialized`** (contract said drop step-level fields). step_index is the EAV-join key AND the `index`-window sort key; it's a coordinate like step_path/vector_index, not step DATA. Keeping it stored (vs reconstructing via the steps join) avoids a join-miss NULL risk in the EAV key and matches the pre-refactor `_VECTOR_KEY`. What WAS dropped from measurements: `step_name` / `step_outcome` / `step_started_at` / `step_ended_at` (from the steps join) and `vector_outcome` (from the vectors join) — the view reconstructs them.
  2. **`ordinal` = 0-based** UNNEST-WITH-ORDINALITY position (`ord - 1`); **`index`** materialized via one shared `_occurrence_index_expr` (a 0-based DENSE_RANK, ORDER BY byte-identical to the old query-time `_INDEX_EXPR`) on measurements + inputs + outputs. The `measurements` view computes the same window for not-yet-materialized inflight rows, so `index` is exposed on both sides and the query-time `_MEASUREMENTS_WITH_INDEX` DENSE_RANK wrap is gone.
  3. **`step_retry` coordinate normalization must be IDENTICAL across the fact and the lane tables** — the real bug that cost the most: the fixtures build vector rows via `RunParquetRow` directly (leaving `step_retry`/`vector_retry` NULL at rest), so `measurements` (which COALESCEs them to 0) and `inputs`/`outputs` (which left them raw NULL) landed on different EAV-join keys → 0 rows. Fixed by giving `_lane_insert` the SAME `COALESCE(step_retry,0)` + step/vector-scope `vector_retry` CASE as `_measurement_unnest_insert`.
  4. **`_lane_insert` switched to `INSERT ... BY NAME`** (was positional). On an UPGRADED on-disk index the new `ordinal`/`index` columns are ALTER-appended at the END, so a positional INSERT misaligns them and silently quarantines the file. Every other insert already used BY NAME; the lane insert was the lone positional one.
  5. **`step_retry` carried on the inflight measurement overlay** (`INFLIGHT_MEASUREMENTS_SCHEMA` + snapshot) so a live measurement matches its finalized row 1:1 and the view exposes it on both sides.
- 2026-07-03 — **Phase 9 (perf gate) — measured, targeted fix applied.** DuckDB does NOT prune the `measurements` view's `steps`/`vectors` reconstruction LEFT JOINs even when an aggregate never reads those columns (confirmed via EXPLAIN + a clean PK-equi-join probe). Controlled micro-benchmark (fixed 30 000-row dataset, fresh DuckDB): unscoped aggregates over the full view cost ~43-47 ms vs ~18 ms over a runs-identity-only view (2.4-2.6×), and the gap grows linearly with rows. So added a lean **`measurement_facts`** view (fact + `runs` join only) and pointed the pure-aggregate metric paths (yield / yield_overall / ppk / trend / retest / time_loss / summary_counts) at it — back to ~phase-6 baseline. `pareto` (needs `step_name`), the parametric/explore surfaces, `distinct_values`, and `get_measurements` keep the full `measurements` view (per-run/per-part scoped, or genuinely need step/vector columns). DB indexes were NOT pruned — the ART `name` indexes on inputs/outputs still serve facet/`distinct_values`/EAV-name lookups; no measurement showed them safe to drop. Full before/after table in the commit + scratchpad bench log.
