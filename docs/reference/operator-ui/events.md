...
| Event type | Pick one of 15 curated event types, or `(any)` for all | The store can hold any event type; this dropdown lists the categories worth a one-click filter — it is **not exhaustive**. Other event types (e.g. `fixture.uut_scanned`, `route.*`, `slot.*`, `stream.*`, `test.record`) only appear under `(any)`. See the [Event types reference](../event-types.md) for the full list. |
