...
| Event type | Pick one of 15 curated event types, or `(any)` for all | The store can hold any event type; this dropdown lists the categories worth a one-click filter — it is **not exhaustive**. Other event types (e.g. `fixture.*`, `route.*`, `slot.*`, `stream.*`) only appear under `(any)`. See the [Event types reference](../event-types.md) for the full list. A few dropdown labels also drift from the emitted names (e.g. `dialog.requested` vs the actual `dialog.opened`); when a one-click filter returns zero rows unexpectedly, switch to `(any)` and confirm the event type column. |
