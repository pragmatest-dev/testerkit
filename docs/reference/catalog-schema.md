...
Authoritative shape of a `catalog/<vendor>/<model>.yaml` entry — the fields, their types, the rules, the decision tree for "where does this datasheet spec go." For worked examples of each pattern (one recipe per recurring datasheet shape), see the [catalog cookbook](catalog-cookbook.md).

**Status:** Frozen at `CATALOG_SCHEMA_VERSION = "1.0"` for the 0.1.0 release.

Source of truth: `src/litmus/models/capability.py` (Pydantic models).
...
Each signal has range, accuracy, resolution, and condition-dependent overrides (`bands` / [SpecBand](models.md) — a value-plus-condition record).
...
Continuous (`range`) or discrete (`options`), with optional condition-dependent overrides via `bands`. Feed SpecBand lookup.
...
Discrete options OR continuous range. Can be referenced in SpecBand `when`. Supports `resolution` (step size) and condition-dependent overrides via `bands`.
...
Attributes can have condition-dependent overrides via `bands`, same pattern as signals.
...
Attributes support exactly one of `value` (numeric or string scalar), `range` (min/max), or `options` (enumerated list). An attribute may also have only `bands` (no base value) when all values are condition-dependent.
...
| `limit_nominal` | Design target, not individually tested |
...
Full enum list: read `MeasurementFunction` in `src/litmus/models/`
...
| Accuracy that varies by frequency/range/mode | `signals.X.bands[]` (SpecBand) |
