...
These three stores are the platform. They're populated automatically by every test run; there's no configuration knob to disable them. See [Three Stores Architecture](../concepts/three-stores.md) and [Event Log](../concepts/event-log.md) for the data model.
