...
Worked recipes for the recurring datasheet shapes you'll meet when authoring `catalog/<vendor>/<model>.yaml`. One recipe per shape — each names the antipattern and the right YAML side-by-side. Read the [catalog schema reference](schema.md) first for the field definitions and the "what goes WHERE" decision tree, then come here for the worked YAML.

## 1. Accuracy by frequency band → SpecBands

```yaml
# Datasheet: AC voltage accuracy varies by frequency
...
```yaml
# Datasheet: reading rate varies by frequency band and acquisition mode
...
#
# RIGHT — SpecBands on a signal (every when: key is a declared sibling;
# the acquisition_mode control matches by option value, not a range):
  signals:
    reading_rate:
...
```yaml
# Datasheet: Frequency sweep time varies by number of frequencies
...
```yaml
# Datasheet: "Residual distortion: 0.004% or −87 dB"
...
These structured values have dedicated models — `AccuracySpec`, `ResolutionSpec`, `RangeSpec`. If a datasheet value fits one, use it — don't store it as a flat `Attribute`.

```yaml
# Datasheet: "Frequency Accuracy: ±0.01% of reading"
...
```yaml
# Datasheet: "Resolution: 6.5 digits"
...
```yaml
# Datasheet: "Harmonic Frequency Range: 40 Hz–50 kHz"
...
## 7. Shared controls — follow the datasheet's "applies to" EXACTLY

```yaml
# A datasheet lists each user-selectable setting against the functions it
# applies to. Put each control ONLY on the listed capabilities — match what
# the datasheet states, don't guess.
#
# Example — settings and the functions they apply to:
#   range → all: dc_voltage, ac_voltage, resistance
#   filter_type → ac_voltage, resistance only
#   averaging_count → dc_voltage only
#
# RIGHT — each control appears ONLY where the datasheet lists it:
...
- function: ac_voltage
  controls:
    range:
      options: [0.1, 1, 10, 100]
      unit: V
    filter_type:           # ac_voltage and resistance only
...
# WRONG — putting filter_type on dc_voltage (datasheet doesn't list it)
# WRONG — putting averaging_count on ac_voltage/resistance (datasheet doesn't list them)
```
...
# It's a capability-level attribute (not a root-level entry attribute) because
...
```yaml
# Device-wide facts → the entry's root attributes: (ONE place, not on capabilities)
# Capability-specific facts → capability attributes (repeated on each applicable cap)
#
# Board-level (root attributes:) — use range for min/max, value for scalars:
attributes:
...
```yaml
# Datasheet: Test current depends on resistance range
#   100 Ω range: 1 mA
#   1 kΩ range: 1 mA
#   10 kΩ range: 100 µA
#   100 kΩ range: 10 µA
...
## 12. Condition ranges must match the datasheet

```yaml
# Datasheet says THD+N accuracy applies to "100 Hz to 20 kHz"
# WRONG:
conditions:
  fundamental_frequency:
...
conditions:
  fundamental_frequency:
    range: {min: 100, max: 20000, unit: Hz}  # matches datasheet
```

## 13. Resolution — match signal unit

```yaml
# Datasheet: "Resolution: 0.0001% or 0.00001 dB"
...
signals:
  distortion:
    accuracy: {absolute: 0.8}
    # No bands needed — accuracy doesn't vary
```
