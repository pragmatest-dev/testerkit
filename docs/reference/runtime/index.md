...
- [`TesterKitClient`](client.md) — Python client that submits test runs (no pytest required). Suits LabVIEW / TestStand bridges.
...
- [HTTP & MCP API](api.md) — REST endpoints exposed by `testerkit serve`, plus the MCP tools. Same shapes either way. Generated from source.
